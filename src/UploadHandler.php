...
class UploadHandler
{
    ...
    protected function gd_create_scaled_image($file_name, $version, $options) {
        ...
        if ($scale >= 1) {
            if ($image_oriented) {
                return $image_quality === null
                    ? $write_func($src_img, $new_file_path)
                    : $write_func($src_img, $new_file_path, $image_quality);
            }
        ...
        $success = imagecopyresampled(
                ...
                intval($img_width),
                intval($img_height)
            ) && ($image_quality === null ? $write_func($new_img, $new_file_path) : $write_func($new_img, $new_file_path, $image_quality));
        $this->gd_set_image_object($file_path, $new_img);
        return $success;
    }
