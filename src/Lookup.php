...
class Lookup
{
    ...
    public function toJson(?DbTableBase $page = null, bool $response = true): array|bool
    {
        ...
        $fldCnt = 0;
        try {
            $stmt = $this->executeQuery($qb, $pageSize, $offset);
            if ($stmt) {
            ...
            if (Config("DEBUG")) {
                LogError($e->getMessage(), ["sql" => $qb->getSQL(), "pageSize" => $pageSize, "offset" => $offset]);
            }
        }
        if (is_array($records)) {
     ...
     * Execute query
     *
     * @param QueryBuilder $qb QueryBuilder of the SQL to be executed
     * @param int $pageSize
     * @param int $offset
     * @return Result
     */
    protected function executeQuery(QueryBuilder $qb, int $pageSize, int $offset): Result
    {
        $tbl = $this->getTable();
        if ($tbl === null) {
            return null;
        ...
        if ($tbl->UseLookupCache) {
            $this->cacheProfile = new QueryCacheProfile(0, "lookup.cache." . $tbl->TableVar . "." . hash("xxh128", $sql), Container("result.cache"));
