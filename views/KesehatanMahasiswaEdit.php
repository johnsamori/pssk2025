...
<form name="fkesehatan_mahasiswaedit" id="fkesehatan_mahasiswaedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
var currentTable = <?= json_encode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { kesehatan_mahasiswa: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fkesehatan_mahasiswaedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fkesehatan_mahasiswaedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["Id_kesehatan", [fields.Id_kesehatan.visible && fields.Id_kesehatan.required ? ew.Validators.required(fields.Id_kesehatan.caption) : null], fields.Id_kesehatan.isInvalid],
...
<?php if ($Page->Id_kesehatan->Visible) { // Id_kesehatan ?>
    <div id="r_Id_kesehatan"<?= $Page->Id_kesehatan->rowAttributes() ?>>
        <label id="elh_kesehatan_mahasiswa_Id_kesehatan" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Id_kesehatan->caption() ?><?= $Page->Id_kesehatan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Id_kesehatan->cellAttributes() ?>>
<span id="el_kesehatan_mahasiswa_Id_kesehatan">
<span<?= $Page->Id_kesehatan->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Id_kesehatan->getDisplayValue($Page->Id_kesehatan->getEditValue()))) ?>"></span>
<input type="hidden" data-table="kesehatan_mahasiswa" data-field="x_Id_kesehatan" data-hidden="1" name="x_Id_kesehatan" id="x_Id_kesehatan" value="<?= HtmlEncode($Page->Id_kesehatan->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fkesehatan_mahasiswaedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fkesehatan_mahasiswaedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fkesehatan_mahasiswaedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
