...
<form name="fkarya_ilmiahedit" id="fkarya_ilmiahedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
var currentTable = <?= json_encode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { karya_ilmiah: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fkarya_ilmiahedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fkarya_ilmiahedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["Id_karya_ilmiah", [fields.Id_karya_ilmiah.visible && fields.Id_karya_ilmiah.required ? ew.Validators.required(fields.Id_karya_ilmiah.caption) : null], fields.Id_karya_ilmiah.isInvalid],
...
<?php if ($Page->Id_karya_ilmiah->Visible) { // Id_karya_ilmiah ?>
    <div id="r_Id_karya_ilmiah"<?= $Page->Id_karya_ilmiah->rowAttributes() ?>>
        <label id="elh_karya_ilmiah_Id_karya_ilmiah" class="<?= $Page->LeftColumnClass ?>"><?= $Page->Id_karya_ilmiah->caption() ?><?= $Page->Id_karya_ilmiah->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->Id_karya_ilmiah->cellAttributes() ?>>
<span id="el_karya_ilmiah_Id_karya_ilmiah">
<span<?= $Page->Id_karya_ilmiah->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->Id_karya_ilmiah->getDisplayValue($Page->Id_karya_ilmiah->getEditValue()))) ?>"></span>
<input type="hidden" data-table="karya_ilmiah" data-field="x_Id_karya_ilmiah" data-hidden="1" name="x_Id_karya_ilmiah" id="x_Id_karya_ilmiah" value="<?= HtmlEncode($Page->Id_karya_ilmiah->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fkarya_ilmiahedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fkarya_ilmiahedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
<script>
loadjs.ready("head", function() { $("#fkarya_ilmiahedit:first *:input[type!=hidden]:first").focus(),$("input").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("select").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()}),$("radio").keydown(function(i){if(13==i.which){var e=$(this).closest("form").find(":input:visible:enabled"),n=e.index(this);n==e.length-1||(e.eq(e.index(this)+1).focus(),i.preventDefault())}else 113==i.which&&$("#btn-action").click()})});
</script>
<?php } ?>
<script<?= Nonce() ?>>
