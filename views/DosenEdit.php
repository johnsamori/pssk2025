...
<form name="fdosenedit" id="fdosenedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
...
<input type="hidden" data-table="dosen" data-field="x_No" data-hidden="1" data-old name="o_No" id="o_No" value="<?= HtmlEncode($Page->No->OldValue ?? $Page->No->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script<?= Nonce() ?>>
loadjs.ready(["fdosenedit", "datetimepicker"], function () {
...
<?php } ?>
<script<?= Nonce() ?>>
loadjs.ready(['fdosenedit', 'jqueryinputmask'], function() {
	options = {
		'jitMasking': false,
		'removeMaskOnSubmit': true
	};
	ew.createjQueryInputMask("fdosenedit", "x_Tanggal_lahir", jQuery.extend(true, "", options));
});
</script>
</span>
</div></div>
