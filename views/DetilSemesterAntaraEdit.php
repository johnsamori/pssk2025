...
<form name="fdetil_semester_antaraedit" id="fdetil_semester_antaraedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script<?= Nonce() ?>>
var currentTable = <?= json_encode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { detil_semester_antara: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fdetil_semester_antaraedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fdetil_semester_antaraedit")
        .setPageId("edit")
...
<?php } ?>
<input type="hidden" data-table="detil_semester_antara" data-field="x_id_smtsr" data-hidden="1" data-old name="o_id_smtsr" id="o_id_smtsr" value="<?= HtmlEncode($Page->id_smtsr->OldValue ?? $Page->id_smtsr->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
<?php if ($Page->no->Visible) { // no ?>
    <div id="r_no"<?= $Page->no->rowAttributes() ?>>
        <label id="elh_detil_semester_antara_no" class="<?= $Page->LeftColumnClass ?>"><?= $Page->no->caption() ?><?= $Page->no->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="detil_semester_antara" data-field="x_no" data-hidden="1" name="x_no" id="x_no" value="<?= HtmlEncode($Page->no->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_detil_semester_antara_NIM">
    <select
        ...
        data-select2-id="fdetil_semester_antaraedit_x_NIM"
        <?php } ?>
...
<?php if (!$Page->NIM->IsNativeSelect) { ?>
<script<?= Nonce() ?>>
loadjs.ready("fdetil_semester_antaraedit", function() {
    var options = { name: "x_NIM", selectId: "fdetil_semester_antaraedit_x_NIM" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdetil_semester_antaraedit.lists.NIM?.lookupOptions.length) {
        options.data = { id: "x_NIM", form: "fdetil_semester_antaraedit" };
    } else {
        options.ajax = { id: "x_NIM", form: "fdetil_semester_antaraedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<input type="hidden" name="fa_x_Bukti_SPP" id= "fa_x_Bukti_SPP" value="<?= (Post("fa_x_Bukti_SPP") == "0") ? "0" : "1" ?>">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdetil_semester_antaraedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdetil_semester_antaraedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
