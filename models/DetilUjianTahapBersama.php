...
/**
 * Table class for detil_ujian_tahap_bersama
 */
class DetilUjianTahapBersama extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "detil_ujian_tahap_bersama"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->Nilai->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
    ...
    public function getMasterFilterFromSession(): string
    {
        ...
        if ($this->getCurrentMasterTable() == "ujian_tahap_bersama") {
            ...
            if ($this->id_utb->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->id_utb, $this->id_utb->getSessionValue(), $masterTable->id_utb->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession(): string
    {
        ...
        if ($this->getCurrentMasterTable() == "ujian_tahap_bersama") {
            $masterTable = Container("ujian_tahap_bersama");
            if ($this->id_utb->getSessionValue() != "") {
                $detailFilter .= "" . GetKeyFilter($this->id_utb, $this->id_utb->getSessionValue(), $masterTable->id_utb->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter(DbTableBase $masterTable, array $keys): ?string
    {
        ...
        switch ($masterTable->TableVar) {
            case "ujian_tahap_bersama":
                $key = $keys["id_utb"] ?? "";
                if (IsEmpty($key)) {
                    if ($masterTable->id_utb->Required) { // Required field and empty value
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->id_utb, $keys["id_utb"], $this->id_utb->DataType, $this->Dbid);
                }
                break;
        }
    ...
    public function getDetailFilter(DbTableBase $masterTable): string
    {
        switch ($masterTable->TableVar) {
            case "ujian_tahap_bersama":
                return GetKeyFilter($this->id_utb, $masterTable->id_utb->DbValue, $masterTable->id_utb->DataType, $masterTable->Dbid);
        }
        return "";
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "detil_ujian_tahap_bersama";
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("detilujiantahapbersamalist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "detilujiantahapbersamaview" => $this->language->phrase("View"),
            "detilujiantahapbersamaedit" => $this->language->phrase("Edit"),
            "detilujiantahapbersamaadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "detilujiantahapbersamalist";
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "DetilUjianTahapBersamaView",
            Config("API_ADD_ACTION") => "DetilUjianTahapBersamaAdd",
            Config("API_EDIT_ACTION") => "DetilUjianTahapBersamaEdit",
            Config("API_DELETE_ACTION") => "DetilUjianTahapBersamaDelete",
            Config("API_LIST_ACTION") => "DetilUjianTahapBersamaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("detilujiantahapbersamaview", $parm);
        } else {
            $url = $this->keyUrl("detilujiantahapbersamaview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "detilujiantahapbersamaadd?" . $parm;
        } else {
            $url = "detilujiantahapbersamaadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("detilujiantahapbersamaedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("detilujiantahapbersamalist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("detilujiantahapbersamaadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("detilujiantahapbersamalist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("detilujiantahapbersamadelete", $parm);
        }
    }
    ...
    {
        if ($this->getCurrentMasterTable() == "ujian_tahap_bersama" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_id_utb", $this->id_utb->getSessionValue()); // Use Session Value
        }
        return $url;
    }
    ...
    public function renderListContent(string $filter)
    {
        global $Response;
        $container = Container();
        $listPage = "DetilUjianTahapBersamaList";
