 ...
 * Page class
 */
class DetilSemesterAntaraView extends DetilSemesterAntara
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "DetilSemesterAntaraView";
    ...
    // CSS class/style
    public string $CurrentPageName = "detilsemesterantaraview";
...
// Is modal
        ...
        if (($keyValue = Get("id_smtsr") ?? Route("id_smtsr")) !== null) {
            ...
            $this->RecKey["id_smtsr"] = $this->id_smtsr->QueryStringValue;
        } elseif (Post("id_smtsr") !== null) {
            $this->id_smtsr->setFormValue(Post("id_smtsr"));
            $this->RecKey["id_smtsr"] = $this->id_smtsr->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->id_smtsr->setQueryStringValue($keyValue);
            $this->RecKey["id_smtsr"] = $this->id_smtsr->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "detilsemesterantaralist"; // Return to list
        }
        if (($keyValue = Get("no") ?? Route("no")) !== null) {
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("detilsemesterantaralist"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->id_smtsr->CurrentValue, $this->CurrentRow['id_smtsr']) && SameString($this->no->CurrentValue, $this->CurrentRow['no'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "detilsemesterantaralist"; // No matching record, return to list
                    } else {
