...
/**
 * Table class for penyakit
 */
class Penyakit extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "penyakit"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_penyakit
        $this->id_penyakit = new DbField(
            $this, // Table
            'x_id_penyakit', // Variable name
            'id_penyakit', // Name
            '`id_penyakit`', // Expression
            '`id_penyakit`', // Basic search expression
            200, // Type
            15, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_penyakit`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_penyakit->InputTextType = "text";
        $this->id_penyakit->Raw = true;
        $this->id_penyakit->IsPrimaryKey = true; // Primary key field
        $this->id_penyakit->Nullable = false; // NOT NULL field
        $this->id_penyakit->Required = true; // Required field
        $this->id_penyakit->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['id_penyakit'] = &$this->id_penyakit;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "penyakit";
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_penyakit', $row)) {
                AddFilter($where, QuotedName('id_penyakit', $this->Dbid) . '=' . QuotedValue($row['id_penyakit'], $this->id_penyakit->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->id_penyakit->DbValue = $row['id_penyakit'];
    ...
    {
        return "`id_penyakit` = '@id_penyakit@'";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_penyakit->CurrentValue : $this->id_penyakit->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_penyakit->CurrentValue = $keys[0];
            } else {
                $this->id_penyakit->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_penyakit', $row) ? $row['id_penyakit'] : null;
        } else {
            $val = !IsEmpty($this->id_penyakit->OldValue) && !$current ? $this->id_penyakit->OldValue : $this->id_penyakit->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@id_penyakit@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("penyakitlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "penyakitview" => $this->language->phrase("View"),
            "penyakitedit" => $this->language->phrase("Edit"),
            "penyakitadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "PenyakitView",
            Config("API_ADD_ACTION") => "PenyakitAdd",
            Config("API_EDIT_ACTION") => "PenyakitEdit",
            Config("API_DELETE_ACTION") => "PenyakitDelete",
            Config("API_LIST_ACTION") => "PenyakitList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("penyakitview", $parm);
        } else {
            $url = $this->keyUrl("penyakitview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("penyakitedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("penyakitlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("penyakitadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("penyakitlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("penyakitdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_penyakit\":" . VarToJson($this->id_penyakit->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_penyakit->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_penyakit->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_penyakit") ?? Route("id_penyakit")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_penyakit->CurrentValue = $key;
            } else {
                $this->id_penyakit->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->id_penyakit);
        ...
        while (($row = $result->fetchAssociative()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->id_penyakit);
