 ...
 * Page class
 */
class PembimbinganView extends Pembimbingan
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "PembimbinganView";
    ...
    // CSS class/style
    public string $CurrentPageName = "pembimbinganview";
...
// Is modal
        ...
        if (($keyValue = Get("id_pem") ?? Route("id_pem")) !== null) {
            ...
            $this->RecKey["id_pem"] = $this->id_pem->QueryStringValue;
        } elseif (Post("id_pem") !== null) {
            $this->id_pem->setFormValue(Post("id_pem"));
            $this->RecKey["id_pem"] = $this->id_pem->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->id_pem->setQueryStringValue($keyValue);
            $this->RecKey["id_pem"] = $this->id_pem->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "pembimbinganlist"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("pembimbinganlist"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->id_pem->CurrentValue, $this->CurrentRow['id_pem'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "pembimbinganlist"; // No matching record, return to list
                    } else {
