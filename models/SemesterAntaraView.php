 ...
 * Page class
 */
class SemesterAntaraView extends SemesterAntara
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "SemesterAntaraView";
    ...
    // CSS class/style
    public string $CurrentPageName = "semesterantaraview";
...
// Is modal
        ...
        if (($keyValue = Get("id_smtr") ?? Route("id_smtr")) !== null) {
            ...
            $this->RecKey["id_smtr"] = $this->id_smtr->QueryStringValue;
        } elseif (Post("id_smtr") !== null) {
            $this->id_smtr->setFormValue(Post("id_smtr"));
            $this->RecKey["id_smtr"] = $this->id_smtr->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->id_smtr->setQueryStringValue($keyValue);
            $this->RecKey["id_smtr"] = $this->id_smtr->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "semesterantaralist"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("semesterantaralist"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->id_smtr->CurrentValue, $this->CurrentRow['id_smtr'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "semesterantaralist"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions(): void
    {
        ...
        */
        ...
        $item = &$option->add("detail_detil_semester_antara");
        $body = $this->language->phrase("ViewPageDetailLink") . $this->language->tablePhrase("detil_semester_antara", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("detilsemesterantaralist?" . Config("TABLE_SHOW_MASTER") . "=semester_antara&" . GetForeignKeyUrl("fk_id_smtr", $this->id_smtr->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'semester_antara')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($this->language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=detil_semester_antara"))) . "\">" . $this->language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'semester_antara')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($this->language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=detil_semester_antara"))) . "\">" . $this->language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'semester_antara')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($this->language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=detil_semester_antara"))) . "\">" . $this->language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
