...
/**
 * Table class for ujian_tahap_bersama
 */
class UjianTahapBersama extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $id_utb;
    public DbField $Ujian;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "ujian_tahap_bersama"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_utb
        $this->id_utb = new DbField(
            $this, // Table
            'x_id_utb', // Variable name
            'id_utb', // Name
            '`id_utb`', // Expression
            '`id_utb`', // Basic search expression
            200, // Type
            10, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_utb`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_utb->InputTextType = "text";
        $this->id_utb->Raw = true;
        $this->id_utb->IsPrimaryKey = true; // Primary key field
        $this->id_utb->IsForeignKey = true; // Foreign key field
        $this->id_utb->Nullable = false; // NOT NULL field
        $this->id_utb->Required = true; // Required field
        $this->id_utb->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['id_utb'] = &$this->id_utb;

        // Ujian
        $this->Ujian = new DbField(
            $this, // Table
            'x_Ujian', // Variable name
            'Ujian', // Name
            '`Ujian`', // Expression
            '`Ujian`', // Basic search expression
            200, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Ujian`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Ujian->InputTextType = "text";
        $this->Ujian->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['Ujian'] = &$this->Ujian;
        ...
        $this->userid->Raw = true;
        $this->userid->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->userid->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
    ...
    public function getDetailUrl(): string
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "detil_ujian_tahap_bersama") {
            $detailUrl = Container("detil_ujian_tahap_bersama")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id_utb", $this->id_utb->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "ujiantahapbersamalist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "ujian_tahap_bersama";
    }
    ...
    {
        // Cascade Update detail table 'detil_ujian_tahap_bersama'
        $cascadeUpdate = false;
        $rowCascade = [];
        if ($old && (isset($row['id_utb']) && $old['id_utb'] != $row['id_utb'])) { // Update detail field 'id_utb'
            $cascadeUpdate = true;
            $rowCascade['id_utb'] = $row['id_utb'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("detil_ujian_tahap_bersama")->loadRecords("`id_utb` = " . QuotedValue($old['id_utb'], DataType::STRING, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $oldDetail) {
                ...
                $success = Container("detil_ujian_tahap_bersama")->rowUpdating($oldDetail, $newDetail);
                if ($success) {
                    $success = Container("detil_ujian_tahap_bersama")->update($rowCascade, $detailKeys, $oldDetail);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("detil_ujian_tahap_bersama")->rowUpdated($oldDetail, $newDetail);
            }
        }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_utb', $row)) {
                AddFilter($where, QuotedName('id_utb', $this->Dbid) . '=' . QuotedValue($row['id_utb'], $this->id_utb->DataType, $this->Dbid));
            }
        }
    ...
    public function delete(array $row, string|array $where = "", bool $currentFilter = false): int|bool
    {
        ...
        // Cascade delete detail table 'detil_ujian_tahap_bersama'
        $detailRows = Container("detil_ujian_tahap_bersama")->loadRecords("`id_utb` = " . QuotedValue($row['id_utb'], DataType::STRING, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($detailRows as $detailRow) {
            $success = Container("detil_ujian_tahap_bersama")->rowDeleting($detailRow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($detailRows as $detailRow) {
                $success = Container("detil_ujian_tahap_bersama")->delete($detailRow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($detailRows as $detailRow) {
                Container("detil_ujian_tahap_bersama")->rowDeleted($detailRow);
            }
        }
        if ($success) {
            try {
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->id_utb->DbValue = $row['id_utb'];
        $this->Ujian->DbValue = $row['Ujian'];
    ...
    {
        $keys = [];
        $val = $current ? $this->id_utb->CurrentValue : $this->id_utb->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_utb->CurrentValue = $keys[0];
            } else {
                $this->id_utb->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_utb', $row) ? $row['id_utb'] : null;
        } else {
            $val = !IsEmpty($this->id_utb->OldValue) && !$current ? $this->id_utb->OldValue : $this->id_utb->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@id_utb@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("ujiantahapbersamalist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "ujiantahapbersamaview" => $this->language->phrase("View"),
            "ujiantahapbersamaedit" => $this->language->phrase("Edit"),
            "ujiantahapbersamaadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "UjianTahapBersamaView",
            Config("API_ADD_ACTION") => "UjianTahapBersamaAdd",
            Config("API_EDIT_ACTION") => "UjianTahapBersamaEdit",
            Config("API_DELETE_ACTION") => "UjianTahapBersamaDelete",
            Config("API_LIST_ACTION") => "UjianTahapBersamaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ujiantahapbersamaview", $parm);
        } else {
            $url = $this->keyUrl("ujiantahapbersamaview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "ujiantahapbersamaadd?" . $parm;
        } else {
            $url = "ujiantahapbersamaadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ujiantahapbersamaedit", $parm);
        } else {
            $url = $this->keyUrl("ujiantahapbersamaedit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ujiantahapbersamalist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ujiantahapbersamaadd", $parm);
        } else {
            $url = $this->keyUrl("ujiantahapbersamaadd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ujiantahapbersamalist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ujiantahapbersamadelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_utb\":" . VarToJson($this->id_utb->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_utb->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_utb->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_utb") ?? Route("id_utb")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_utb->CurrentValue = $key;
            } else {
                $this->id_utb->OldValue = $key;
            }
    ...
    public function renderListContent(string $filter)
    {
        global $Response;
        $container = Container();
        $listPage = "UjianTahapBersamaList";
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->id_utb);
                    $doc->exportCaption($this->Ujian);
        ...
        while (($row = $result->fetchAssociative()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->id_utb);
                        $doc->exportField($this->Ujian);
