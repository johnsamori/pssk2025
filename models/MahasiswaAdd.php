 ...
 * Page class
 */
class MahasiswaAdd extends Mahasiswa
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "MahasiswaAdd";
    ...
    // CSS class/style
    public string $CurrentPageName = "mahasiswaadd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                if ($this->addRow($oldRow)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "mahasiswaview") {
...
// Get upload files
    ...
    protected function addRow(?array $oldRow = null): bool
    {
        ...
        if ($this->NIM->CurrentValue != "") { // Check field with unique index
            $filter = "(`NIM` = '" . AdjustSql($this->NIM->CurrentValue) . "')";
        ...
        if ($insertRow && $this->ValidateKey && strval($newRow['NIM']) == "") {
    ...
    protected function getAddRow(): array
    {
        ...
        // Provinsi_Tempat_Lahir
        $this->Provinsi_Tempat_Lahir->setDbValueDef($newRow, $this->Provinsi_Tempat_Lahir->CurrentValue, false);

        // Kota_Tempat_Lahir
        $this->Kota_Tempat_Lahir->setDbValueDef($newRow, $this->Kota_Tempat_Lahir->CurrentValue, false);
        ...
        // Golongan_Darah
        $this->Golongan_Darah->setDbValueDef($newRow, $this->Golongan_Darah->CurrentValue, false);

        // Tinggi_Badan
        $this->Tinggi_Badan->setDbValueDef($newRow, $this->Tinggi_Badan->CurrentValue, false);

        // Berat_Badan
        $this->Berat_Badan->setDbValueDef($newRow, $this->Berat_Badan->CurrentValue, false);

        // Asal_sekolah
        $this->Asal_sekolah->setDbValueDef($newRow, $this->Asal_sekolah->CurrentValue, false);

        // Tahun_Ijazah
        $this->Tahun_Ijazah->setDbValueDef($newRow, $this->Tahun_Ijazah->CurrentValue, false);

        // Nomor_Ijazah
        $this->Nomor_Ijazah->setDbValueDef($newRow, $this->Nomor_Ijazah->CurrentValue, false);

        // Nilai_Raport_Kelas_10
        $this->Nilai_Raport_Kelas_10->setDbValueDef($newRow, $this->Nilai_Raport_Kelas_10->CurrentValue, false);

        // Nilai_Raport_Kelas_11
        $this->Nilai_Raport_Kelas_11->setDbValueDef($newRow, $this->Nilai_Raport_Kelas_11->CurrentValue, false);

        // Nilai_Raport_Kelas_12
        $this->Nilai_Raport_Kelas_12->setDbValueDef($newRow, $this->Nilai_Raport_Kelas_12->CurrentValue, false);

        // Tanggal_Daftar
        $this->Tanggal_Daftar->setDbValueDef($newRow, UnFormatDateTime($this->Tanggal_Daftar->CurrentValue, $this->Tanggal_Daftar->formatPattern()), false);

        // No_Test
        $this->No_Test->setDbValueDef($newRow, $this->No_Test->CurrentValue, false);

        // Status_Masuk
        $this->Status_Masuk->setDbValueDef($newRow, $this->Status_Masuk->CurrentValue, false);

        // Jalur_Masuk
        $this->Jalur_Masuk->setDbValueDef($newRow, $this->Jalur_Masuk->CurrentValue, false);

        // Bukti_Lulus
        $this->Bukti_Lulus->setDbValueDef($newRow, $this->Bukti_Lulus->CurrentValue, false);

        // Tes_Potensi_Akademik
        $this->Tes_Potensi_Akademik->setDbValueDef($newRow, $this->Tes_Potensi_Akademik->CurrentValue, false);

        // Tes_Wawancara
        $this->Tes_Wawancara->setDbValueDef($newRow, $this->Tes_Wawancara->CurrentValue, false);

        // Tes_Kesehatan
        $this->Tes_Kesehatan->setDbValueDef($newRow, $this->Tes_Kesehatan->CurrentValue, false);

        // Hasil_Test_Kesehatan
        $this->Hasil_Test_Kesehatan->setDbValueDef($newRow, $this->Hasil_Test_Kesehatan->CurrentValue, false);

        // Test_MMPI
        $this->Test_MMPI->setDbValueDef($newRow, $this->Test_MMPI->CurrentValue, false);

        // Hasil_Test_MMPI
        $this->Hasil_Test_MMPI->setDbValueDef($newRow, $this->Hasil_Test_MMPI->CurrentValue, false);

        // Angkatan
        $this->Angkatan->setDbValueDef($newRow, $this->Angkatan->CurrentValue, false);

        // Tarif_SPP
        $this->Tarif_SPP->setDbValueDef($newRow, $this->Tarif_SPP->CurrentValue, false);

        // NIK_No_KTP
        $this->NIK_No_KTP->setDbValueDef($newRow, $this->NIK_No_KTP->CurrentValue, false);

        // No_KK
        $this->No_KK->setDbValueDef($newRow, $this->No_KK->CurrentValue, false);

        // NPWP
        $this->NPWP->setDbValueDef($newRow, $this->NPWP->CurrentValue, false);

        // Status_Nikah
        $this->Status_Nikah->setDbValueDef($newRow, $this->Status_Nikah->CurrentValue, false);

        // Kewarganegaraan
        $this->Kewarganegaraan->setDbValueDef($newRow, $this->Kewarganegaraan->CurrentValue, false);

        // Propinsi_Tempat_Tinggal
        $this->Propinsi_Tempat_Tinggal->setDbValueDef($newRow, $this->Propinsi_Tempat_Tinggal->CurrentValue, false);

        // Kota_Tempat_Tinggal
        $this->Kota_Tempat_Tinggal->setDbValueDef($newRow, $this->Kota_Tempat_Tinggal->CurrentValue, false);

        // Kecamatan_Tempat_Tinggal
        $this->Kecamatan_Tempat_Tinggal->setDbValueDef($newRow, $this->Kecamatan_Tempat_Tinggal->CurrentValue, false);

        // Alamat_Tempat_Tinggal
        $this->Alamat_Tempat_Tinggal->setDbValueDef($newRow, $this->Alamat_Tempat_Tinggal->CurrentValue, false);

        // RT
        $this->RT->setDbValueDef($newRow, $this->RT->CurrentValue, false);

        // RW
        $this->RW->setDbValueDef($newRow, $this->RW->CurrentValue, false);

        // Kelurahan
        $this->Kelurahan->setDbValueDef($newRow, $this->Kelurahan->CurrentValue, false);

        // Kode_Pos
        $this->Kode_Pos->setDbValueDef($newRow, $this->Kode_Pos->CurrentValue, false);

        // Nomor_Telpon_HP
        $this->Nomor_Telpon_HP->setDbValueDef($newRow, $this->Nomor_Telpon_HP->CurrentValue, false);

        // Email
        $this->_Email->setDbValueDef($newRow, $this->_Email->CurrentValue, false);

        // Jenis_Tinggal
        $this->Jenis_Tinggal->setDbValueDef($newRow, $this->Jenis_Tinggal->CurrentValue, false);

        // Alat_Transportasi
        $this->Alat_Transportasi->setDbValueDef($newRow, $this->Alat_Transportasi->CurrentValue, false);

        // Sumber_Dana
        $this->Sumber_Dana->setDbValueDef($newRow, $this->Sumber_Dana->CurrentValue, false);

        // Sumber_Dana_Beasiswa
        $this->Sumber_Dana_Beasiswa->setDbValueDef($newRow, $this->Sumber_Dana_Beasiswa->CurrentValue, false);

        // Jumlah_Sudara
        $this->Jumlah_Sudara->setDbValueDef($newRow, $this->Jumlah_Sudara->CurrentValue, false);

        // Status_Bekerja
        $this->Status_Bekerja->setDbValueDef($newRow, $this->Status_Bekerja->CurrentValue, false);

        // Nomor_Asuransi
        $this->Nomor_Asuransi->setDbValueDef($newRow, $this->Nomor_Asuransi->CurrentValue, false);

        // Hobi
        $this->Hobi->setDbValueDef($newRow, $this->Hobi->CurrentValue, false);

        // Foto
        $this->Foto->setDbValueDef($newRow, $this->Foto->CurrentValue, false);

        // Nama_Ayah
        $this->Nama_Ayah->setDbValueDef($newRow, $this->Nama_Ayah->CurrentValue, false);

        // Pekerjaan_Ayah
        $this->Pekerjaan_Ayah->setDbValueDef($newRow, $this->Pekerjaan_Ayah->CurrentValue, false);

        // Nama_Ibu
        $this->Nama_Ibu->setDbValueDef($newRow, $this->Nama_Ibu->CurrentValue, false);

        // Pekerjaan_Ibu
        $this->Pekerjaan_Ibu->setDbValueDef($newRow, $this->Pekerjaan_Ibu->CurrentValue, false);

        // Alamat_Orang_Tua
        $this->Alamat_Orang_Tua->setDbValueDef($newRow, $this->Alamat_Orang_Tua->CurrentValue, false);

        // e_mail_Oranng_Tua
        $this->e_mail_Oranng_Tua->setDbValueDef($newRow, $this->e_mail_Oranng_Tua->CurrentValue, false);

        // No_Kontak_Orang_Tua
        $this->No_Kontak_Orang_Tua->setDbValueDef($newRow, $this->No_Kontak_Orang_Tua->CurrentValue, false);
        ...
        $this->tanggal_input->setDbValueDef($newRow, UnFormatDateTime($this->tanggal_input->CurrentValue, $this->tanggal_input->formatPattern()), false);
        return $newRow;
    }
