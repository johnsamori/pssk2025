 ...
 * Page class
 */
class UjianTahapBersamaList extends UjianTahapBersama
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "UjianTahapBersamaList";
    ...
    public string $FormName = "fujian_tahap_bersamalist";

    // CSS class/style
    public string $CurrentPageName = "ujiantahapbersamalist";
    ...
    {
        $this->id_utb->setVisibility();
        $this->Ujian->setVisibility();
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->TableVar = 'ujian_tahap_bersama';
        $this->TableName = 'ujian_tahap_bersama';
        ...
        // Table object (ujian_tahap_bersama)
        if (!isset($GLOBALS["ujian_tahap_bersama"]) || $GLOBALS["ujian_tahap_bersama"]::class == PROJECT_NAMESPACE . "ujian_tahap_bersama") {
            $GLOBALS["ujian_tahap_bersama"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "ujiantahapbersamaadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "ujiantahapbersamadelete";
        $this->MultiUpdateUrl = "ujiantahapbersamaupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'ujian_tahap_bersama');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "ujiantahapbersamaview"); // If View page, no primary button
                } else { // List page
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "fujian_tahap_bersamagrid";
        }
		...
		if (ReadCookie('ujian_tahap_bersama_searchpanel') == 'notactive' || ReadCookie('ujian_tahap_bersama_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('ujian_tahap_bersama_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fujian_tahap_bersamasrch");
        }
        $filterList = Concat($filterList, $this->id_utb->AdvancedSearch->toJson(), ","); // Field id_utb
        $filterList = Concat($filterList, $this->Ujian->AdvancedSearch->toJson(), ","); // Field Ujian
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fujian_tahap_bersamasrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field id_utb
        $this->id_utb->AdvancedSearch->SearchValue = $filter["x_id_utb"] ?? "";
        $this->id_utb->AdvancedSearch->SearchOperator = $filter["z_id_utb"] ?? "";
        $this->id_utb->AdvancedSearch->SearchCondition = $filter["v_id_utb"] ?? "";
        $this->id_utb->AdvancedSearch->SearchValue2 = $filter["y_id_utb"] ?? "";
        $this->id_utb->AdvancedSearch->SearchOperator2 = $filter["w_id_utb"] ?? "";
        $this->id_utb->AdvancedSearch->save();

        // Field Ujian
        $this->Ujian->AdvancedSearch->SearchValue = $filter["x_Ujian"] ?? "";
        $this->Ujian->AdvancedSearch->SearchOperator = $filter["z_Ujian"] ?? "";
        $this->Ujian->AdvancedSearch->SearchCondition = $filter["v_Ujian"] ?? "";
        $this->Ujian->AdvancedSearch->SearchValue2 = $filter["y_Ujian"] ?? "";
        $this->Ujian->AdvancedSearch->SearchOperator2 = $filter["w_Ujian"] ?? "";
        $this->Ujian->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->id_utb;
        $searchFlds[] = &$this->Ujian;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_utb); // id_utb
            $this->updateSort($this->Ujian); // Ujian
    ...
    protected function setupListOptions(): void
    {
        ...
        // "detail_detil_ujian_tahap_bersama"
        $item = &$this->ListOptions->add("detail_detil_ujian_tahap_bersama");
        $item->CssClass = "text-nowrap";
        $item->Visible = $this->security->allowList(CurrentProjectID() . 'detil_ujian_tahap_bersama');
        $item->OnLeft = true;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("detil_ujian_tahap_bersama");
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"ujian_tahap_bersama\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"ujian_tahap_bersama\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"ujian_tahap_bersama\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fujian_tahap_bersamalist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fujian_tahap_bersamalist\" data-key=\"" . $this->keyToJson(true) .
        ...
        // "detail_detil_ujian_tahap_bersama"
        $opt = $this->ListOptions["detail_detil_ujian_tahap_bersama"];
        if ($this->security->allowList(CurrentProjectID() . 'detil_ujian_tahap_bersama')) {
            $body = $this->language->phrase("DetailLink") . $this->language->tablePhrase("detil_ujian_tahap_bersama", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("detilujiantahapbersamalist?" . Config("TABLE_SHOW_MASTER") . "=ujian_tahap_bersama&" . GetForeignKeyUrl("fk_id_utb", $this->id_utb->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("DetilUjianTahapBersamaGrid");
            if ($detailPage->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'ujian_tahap_bersama')) {
                $caption = $this->language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=detil_ujian_tahap_bersama");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "detil_ujian_tahap_bersama";
            }
            if ($detailPage->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'ujian_tahap_bersama')) {
                $caption = $this->language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=detil_ujian_tahap_bersama");
                ...
                $detailEditTblVar .= "detil_ujian_tahap_bersama";
            }
            if ($detailPage->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'ujian_tahap_bersama')) {
                $caption = $this->language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=detil_ujian_tahap_bersama");
                ...
                $detailCopyTblVar .= "detil_ujian_tahap_bersama";
            }
            if ($links != "") {
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_utb->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["id_utb"] = strval($this->id_utb->DbValue);

        // Column "detail_detil_ujian_tahap_bersama"
        if ($this->DetailPages?->getItem("detil_ujian_tahap_bersama")?->Visible && $this->security->allowList(CurrentProjectID() . 'detil_ujian_tahap_bersama')) {
            $link = "";
            $option = $this->ListOptions["detail_detil_ujian_tahap_bersama"];
            $detailTbl = Container("detil_ujian_tahap_bersama");
            ...
            $url = "detilujiantahapbersamapreview?t=ujian_tahap_bersama&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"detil_ujian_tahap_bersama\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group btn-group btn-group-sm d-none\">";
            if ($this->security->allowList(CurrentProjectID() . 'ujian_tahap_bersama')) {
                $label = $this->language->tablePhrase("detil_ujian_tahap_bersama", "TblCaption");

        		// Begin of modification by Masino Sinaga, December 11, 2024
                // $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"detil_ujian_tahap_bersama\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                ...
                $detaillnk = GetUrl("detilujiantahapbersamalist?" . Config("TABLE_SHOW_MASTER") . "=ujian_tahap_bersama");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $this->language->tablePhrase("detil_ujian_tahap_bersama", "TblCaption");
            ...
            $detailPageObj = Container("DetilUjianTahapBersamaGrid");
            if ($detailPageObj->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'ujian_tahap_bersama')) {
                $caption = $this->language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=detil_ujian_tahap_bersama"));
            ...
            if ($detailPageObj->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'ujian_tahap_bersama')) {
                $caption = $this->language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=detil_ujian_tahap_bersama"));
            ...
            if ($detailPageObj->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'ujian_tahap_bersama')) {
                $caption = $this->language->phrase("MasterDetailCopyLink");
                $copyurl = GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=detil_ujian_tahap_bersama"));
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"ujian_tahap_bersama\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_detil_ujian_tahap_bersama");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=detil_ujian_tahap_bersama");
        $detailPage = Container("DetilUjianTahapBersamaGrid");
        $caption = $this->language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $this->security->allowAdd(CurrentProjectID() . 'ujian_tahap_bersama') && $this->security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "detil_ujian_tahap_bersama";
        }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "id_utb");
            $this->createColumnOption($option, "Ujian");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fujian_tahap_bersamasrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fujian_tahap_bersamasrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fujian_tahap_bersamalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_ujian_tahap_bersama", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_ujian_tahap_bersama",
...
// Load basic search values
    ...
    public function loadRowValues(array|bool|null $row = null): void
    {
        ...
        $this->id_utb->setDbValue($row['id_utb']);
        $this->Ujian->setDbValue($row['Ujian']);
    ...
    {
        $row = [];
        $row['id_utb'] = $this->id_utb->DefaultValue;
        $row['Ujian'] = $this->Ujian->DefaultValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // id_utb
            $this->id_utb->ViewValue = $this->id_utb->CurrentValue;

            // Ujian
            $this->Ujian->ViewValue = $this->Ujian->CurrentValue;
            ...
            $this->userid->ViewValue = FormatNumber($this->userid->ViewValue, $this->userid->formatPattern());
            ...
            // id_utb
            $this->id_utb->HrefValue = "";
            $this->id_utb->TooltipValue = "";

            // Ujian
            $this->Ujian->HrefValue = "";
            $this->Ujian->TooltipValue = "";
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('ujian_tahap_bersama_searchpanel') == 'notactive' || ReadCookie('ujian_tahap_bersama_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fujian_tahap_bersamasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('ujian_tahap_bersama_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fujian_tahap_bersamasrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fujian_tahap_bersamasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
