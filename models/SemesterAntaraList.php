 ...
 * Page class
 */
class SemesterAntaraList extends SemesterAntara
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "SemesterAntaraList";
    ...
    public string $FormName = "fsemester_antaralist";

    // CSS class/style
    public string $CurrentPageName = "semesterantaralist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Initialize URLs
        $this->AddUrl = "semesterantaraadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "semesterantaradelete";
        $this->MultiUpdateUrl = "semesterantaraupdate";
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "fsemester_antaragrid";
        }
		...
		if (ReadCookie('semester_antara_searchpanel') == 'notactive' || ReadCookie('semester_antara_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('semester_antara_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fsemester_antarasrch");
        }
        $filterList = Concat($filterList, $this->id_smtr->AdvancedSearch->toJson(), ","); // Field id_smtr
        $filterList = Concat($filterList, $this->Semester->AdvancedSearch->toJson(), ","); // Field Semester
        $filterList = Concat($filterList, $this->Jadwal->AdvancedSearch->toJson(), ","); // Field Jadwal
        $filterList = Concat($filterList, $this->Tahun_Akademik->AdvancedSearch->toJson(), ","); // Field Tahun_Akademik
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsemester_antarasrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field id_smtr
        $this->id_smtr->AdvancedSearch->SearchValue = $filter["x_id_smtr"] ?? "";
        $this->id_smtr->AdvancedSearch->SearchOperator = $filter["z_id_smtr"] ?? "";
        $this->id_smtr->AdvancedSearch->SearchCondition = $filter["v_id_smtr"] ?? "";
        $this->id_smtr->AdvancedSearch->SearchValue2 = $filter["y_id_smtr"] ?? "";
        $this->id_smtr->AdvancedSearch->SearchOperator2 = $filter["w_id_smtr"] ?? "";
        $this->id_smtr->AdvancedSearch->save();
        ...
        // Field Jadwal
        $this->Jadwal->AdvancedSearch->SearchValue = $filter["x_Jadwal"] ?? "";
        $this->Jadwal->AdvancedSearch->SearchOperator = $filter["z_Jadwal"] ?? "";
        $this->Jadwal->AdvancedSearch->SearchCondition = $filter["v_Jadwal"] ?? "";
        $this->Jadwal->AdvancedSearch->SearchValue2 = $filter["y_Jadwal"] ?? "";
        $this->Jadwal->AdvancedSearch->SearchOperator2 = $filter["w_Jadwal"] ?? "";
        $this->Jadwal->AdvancedSearch->save();

        // Field Tahun_Akademik
        $this->Tahun_Akademik->AdvancedSearch->SearchValue = $filter["x_Tahun_Akademik"] ?? "";
        $this->Tahun_Akademik->AdvancedSearch->SearchOperator = $filter["z_Tahun_Akademik"] ?? "";
        $this->Tahun_Akademik->AdvancedSearch->SearchCondition = $filter["v_Tahun_Akademik"] ?? "";
        $this->Tahun_Akademik->AdvancedSearch->SearchValue2 = $filter["y_Tahun_Akademik"] ?? "";
        $this->Tahun_Akademik->AdvancedSearch->SearchOperator2 = $filter["w_Tahun_Akademik"] ?? "";
        $this->Tahun_Akademik->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->id_smtr;
        $searchFlds[] = &$this->Semester;
        $searchFlds[] = &$this->Jadwal;
        $searchFlds[] = &$this->Tahun_Akademik;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_smtr); // id_smtr
            $this->updateSort($this->Semester); // Semester
            $this->updateSort($this->Jadwal); // Jadwal
            $this->updateSort($this->Tahun_Akademik); // Tahun_Akademik
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id_smtr->setSort("");
                $this->Semester->setSort("");
                $this->Jadwal->setSort("");
                $this->Tahun_Akademik->setSort("");
    ...
    protected function setupListOptions(): void
    {
        ...
        // "detail_detil_semester_antara"
        $item = &$this->ListOptions->add("detail_detil_semester_antara");
        $item->CssClass = "text-nowrap";
        $item->Visible = $this->security->allowList(CurrentProjectID() . 'detil_semester_antara');
        $item->OnLeft = true;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("detil_semester_antara");
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"semester_antara\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"semester_antara\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"semester_antara\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsemester_antaralist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fsemester_antaralist\" data-key=\"" . $this->keyToJson(true) .
        ...
        // "detail_detil_semester_antara"
        $opt = $this->ListOptions["detail_detil_semester_antara"];
        if ($this->security->allowList(CurrentProjectID() . 'detil_semester_antara')) {
            $body = $this->language->phrase("DetailLink") . $this->language->tablePhrase("detil_semester_antara", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("detilsemesterantaralist?" . Config("TABLE_SHOW_MASTER") . "=semester_antara&" . GetForeignKeyUrl("fk_id_smtr", $this->id_smtr->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("DetilSemesterAntaraGrid");
            if ($detailPage->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'semester_antara')) {
                $caption = $this->language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=detil_semester_antara");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "detil_semester_antara";
            }
            if ($detailPage->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'semester_antara')) {
                $caption = $this->language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=detil_semester_antara");
                ...
                $detailEditTblVar .= "detil_semester_antara";
            }
            if ($detailPage->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'semester_antara')) {
                $caption = $this->language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=detil_semester_antara");
                ...
                $detailCopyTblVar .= "detil_semester_antara";
            }
            if ($links != "") {
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_smtr->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["id_smtr"] = strval($this->id_smtr->DbValue);

        // Column "detail_detil_semester_antara"
        if ($this->DetailPages?->getItem("detil_semester_antara")?->Visible && $this->security->allowList(CurrentProjectID() . 'detil_semester_antara')) {
            $link = "";
            $option = $this->ListOptions["detail_detil_semester_antara"];
            $detailTbl = Container("detil_semester_antara");
            ...
            $url = "detilsemesterantarapreview?t=semester_antara&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"detil_semester_antara\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group btn-group btn-group-sm d-none\">";
            if ($this->security->allowList(CurrentProjectID() . 'semester_antara')) {
                $label = $this->language->tablePhrase("detil_semester_antara", "TblCaption");

        		// Begin of modification by Masino Sinaga, December 11, 2024
                // $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"detil_semester_antara\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                ...
                $detaillnk = GetUrl("detilsemesterantaralist?" . Config("TABLE_SHOW_MASTER") . "=semester_antara");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $this->language->tablePhrase("detil_semester_antara", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'semester_antara')) {
                $caption = $this->language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=detil_semester_antara"));
            ...
            if ($detailPageObj->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'semester_antara')) {
                $caption = $this->language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=detil_semester_antara"));
            ...
            if ($detailPageObj->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'semester_antara')) {
                $caption = $this->language->phrase("MasterDetailCopyLink");
                $copyurl = GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=detil_semester_antara"));
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"semester_antara\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_detil_semester_antara");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=detil_semester_antara");
        $detailPage = Container("DetilSemesterAntaraGrid");
        $caption = $this->language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $this->security->allowAdd(CurrentProjectID() . 'semester_antara') && $this->security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "detil_semester_antara";
        }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "id_smtr");
            $this->createColumnOption($option, "Semester");
            $this->createColumnOption($option, "Jadwal");
            $this->createColumnOption($option, "Tahun_Akademik");
            $this->createColumnOption($option, "Tanggal");
            $this->createColumnOption($option, "User_id");
            $this->createColumnOption($option, "User");
            $this->createColumnOption($option, "IP");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fsemester_antarasrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fsemester_antarasrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fsemester_antaralist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_semester_antara", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_semester_antara",
...
// Load basic search values
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('semester_antara_searchpanel') == 'notactive' || ReadCookie('semester_antara_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsemester_antarasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('semester_antara_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsemester_antarasrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fsemester_antarasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
