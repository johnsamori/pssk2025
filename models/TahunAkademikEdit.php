 ...
 * Page class
 */
class TahunAkademikEdit extends TahunAkademik
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "TahunAkademikEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "tahunakademikedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_tahun") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_tahun->setQueryStringValue($keyValue);
                $this->id_tahun->setOldValue($this->id_tahun->QueryStringValue);
            } elseif (Post("id_tahun") !== null) {
                $this->id_tahun->setFormValue(Post("id_tahun"));
                $this->id_tahun->setOldValue($this->id_tahun->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("id_tahun") ?? Route("id_tahun")) !== null) {
                    $this->id_tahun->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->id_tahun->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("tahunakademiklist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "tahunakademiklist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "tahunakademiklist") {
                            ...
                            $returnUrl = "tahunakademiklist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'id_tahun' before field var 'x_id_tahun'
        $val = $this->getFormValue("id_tahun", null) ?? $this->getFormValue("x_id_tahun", null);
        if (!$this->id_tahun->IsDetailKey) {
            $this->id_tahun->setFormValue($val);
        }
    ...
    {
        $this->id_tahun->CurrentValue = $this->id_tahun->FormValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // id_tahun
            $this->id_tahun->setupEditAttributes();
            $this->id_tahun->EditValue = $this->id_tahun->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->id_tahun->Visible && $this->id_tahun->Required) {
                if (!$this->id_tahun->IsDetailKey && IsEmpty($this->id_tahun->FormValue)) {
                    $this->id_tahun->addErrorMessage(str_replace("%s", $this->id_tahun->caption(), $this->id_tahun->RequiredErrorMessage));
                }
            }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("tahunakademiklist"), "", $this->TableVar, true);
        $pageId = "edit";
