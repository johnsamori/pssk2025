 ...
 * Page class
 */
class UjianTahapBersamaEdit extends UjianTahapBersama
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "UjianTahapBersamaEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "ujiantahapbersamaedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_utb") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_utb->setQueryStringValue($keyValue);
                $this->id_utb->setOldValue($this->id_utb->QueryStringValue);
            } elseif (Post("id_utb") !== null) {
                $this->id_utb->setFormValue(Post("id_utb"));
                $this->id_utb->setOldValue($this->id_utb->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("ujiantahapbersamalist"); // No matching record, return to list
                        return;
                    }
            ...
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "ujiantahapbersamalist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ujiantahapbersamalist") {
                            ...
                            $returnUrl = "ujiantahapbersamalist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'Ujian' before field var 'x_Ujian'
        $val = $this->getFormValue("Ujian", null) ?? $this->getFormValue("x_Ujian", null);
        if (!$this->Ujian->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->Ujian->Visible = false; // Disable update for API request
            } else {
                $this->Ujian->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->Ujian->CurrentValue = $this->Ujian->FormValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // Ujian
            $this->Ujian->setupEditAttributes();
            $this->Ujian->EditValue = !$this->Ujian->Raw ? HtmlDecode($this->Ujian->CurrentValue) : $this->Ujian->CurrentValue;
            $this->Ujian->PlaceHolder = RemoveHtml($this->Ujian->caption());
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->Ujian->Visible && $this->Ujian->Required) {
                if (!$this->Ujian->IsDetailKey && IsEmpty($this->Ujian->FormValue)) {
                    $this->Ujian->addErrorMessage(str_replace("%s", $this->Ujian->caption(), $this->Ujian->RequiredErrorMessage));
                }
            }
        ...
        if (in_array("detil_ujian_tahap_bersama", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
    ...
    protected function editRow(): bool
    {
        ...
        // Check field with unique index (id_utb)
        if ($this->id_utb->CurrentValue != "") {
            $filterChk = "(`id_utb` = '" . AdjustSql($this->id_utb->CurrentValue) . "')";
            ...
            if ($rsChk->fetchAssociative()) {
                $idxErrMsg = sprintf($this->language->phrase("DuplicateIndex"), $this->id_utb->CurrentValue, $this->id_utb->caption());
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("DetilUjianTahapBersamaGrid");
            if (in_array("detil_ujian_tahap_bersama", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $this->security->loadCurrentUserLevel($this->ProjectID . "detil_ujian_tahap_bersama"); // Load user level of detail table
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // Ujian
        $this->Ujian->setDbValueDef($newRow, $this->Ujian->CurrentValue, $this->Ujian->ReadOnly);
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['Ujian'])) { // Ujian
            $this->Ujian->CurrentValue = $row['Ujian'];
        }
