...
/**
 * Table class for daftar matakuliah
 */
class DaftarMatakuliah extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "`daftar matakuliah`"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // SKS
        $this->SKS = new DbField(
            $this, // Table
            'x_SKS', // Variable name
            'SKS', // Name
            '`SKS`', // Expression
            '`SKS`', // Basic search expression
            200, // Type
            255, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`SKS`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->SKS->InputTextType = "text";
        $this->SKS->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`daftar matakuliah`";
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('Kode', $row)) {
                AddFilter($where, QuotedName('Kode', $this->Dbid) . '=' . QuotedValue($row['Kode'], $this->Kode->DataType, $this->Dbid));
            }
        }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            ...
            $val = !IsEmpty($this->Kode->OldValue) && !$current ? $this->Kode->OldValue : $this->Kode->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@Kode@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("daftarmatakuliahlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "daftarmatakuliahview" => $this->language->phrase("View"),
            "daftarmatakuliahedit" => $this->language->phrase("Edit"),
            "daftarmatakuliahadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "DaftarMatakuliahView",
            Config("API_ADD_ACTION") => "DaftarMatakuliahAdd",
            Config("API_EDIT_ACTION") => "DaftarMatakuliahEdit",
            Config("API_DELETE_ACTION") => "DaftarMatakuliahDelete",
            Config("API_LIST_ACTION") => "DaftarMatakuliahList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("daftarmatakuliahview", $parm);
        } else {
            $url = $this->keyUrl("daftarmatakuliahview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "daftarmatakuliahadd?" . $parm;
        } else {
            $url = "daftarmatakuliahadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("daftarmatakuliahedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("daftarmatakuliahlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("daftarmatakuliahadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("daftarmatakuliahlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("daftarmatakuliahdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"Kode\":" . VarToJson($this->Kode->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl(string $url, string $parm = ""): string
    {
        if ($this->Kode->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->Kode->CurrentValue);
        } else {
