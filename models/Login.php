...
// Is modal
        ...
        if (IsLoggingIn()) { // After authorized by 2FA
            ...
            $this->security->validateUser($this->Username->CurrentValue);
            Session(SESSION_USER_PROFILE_USER_NAME, "");
            $valid = true;
            ...
            if ($valid) {
                $valid = $this->security->validateUser($this->Username->CurrentValue, $this->Password->CurrentValue ?? ""); // Manual login
                if (!$valid) {
        ...
        $profile = Profile();
        if ($valid && !IsSysAdmin() && !IsImpersonator() && !$profile->passwordExpired() && !$profile->isValidUser(SessionId())) {
