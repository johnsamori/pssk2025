 ...
 * Page class
 */
class DetilSemesterAntaraEdit extends DetilSemesterAntara
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "DetilSemesterAntaraEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "detilsemesterantaraedit";
...
// Is modal
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("id_smtsr") ?? Key(0) ?? Route(2)) !== null) {
                $this->id_smtsr->setQueryStringValue($keyValue);
                $this->id_smtsr->setOldValue($this->id_smtsr->QueryStringValue);
            } elseif (Post("id_smtsr") !== null) {
                $this->id_smtsr->setFormValue(Post("id_smtsr"));
                $this->id_smtsr->setOldValue($this->id_smtsr->FormValue);
            } else {
                $loaded = false; // Unable to load key
            }
            if (($keyValue = Get("no") ?? Key(1) ?? Route(3)) !== null) {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("id_smtsr") ?? Route("id_smtsr")) !== null) {
                    $this->id_smtsr->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->id_smtsr->CurrentValue = null;
                }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("detilsemesterantaralist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "detilsemesterantaralist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions && !$this->getCurrentMasterTable()) {
                        ...
                        if (GetPageName($returnUrl) != "detilsemesterantaralist") {
                            ...
                            $returnUrl = "detilsemesterantaralist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    public function renderRow(): void
    {
        ...
        // KRS
        $this->KRS->RowCssClass = "row";

        // Bukti_SPP
        $this->Bukti_SPP->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->isShow()) {
                $this->Bukti_SPP->Upload->setupTempDirectory();
            }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("detilsemesterantaralist"), "", $this->TableVar, true);
        $pageId = "edit";
