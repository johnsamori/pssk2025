...
/**
 * Table class for mata_kuliah
 */
class MataKuliah extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "mata_kuliah"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_mk
        $this->id_mk = new DbField(
            $this, // Table
            'x_id_mk', // Variable name
            'id_mk', // Name
            '`id_mk`', // Expression
            '`id_mk`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_mk`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_mk->InputTextType = "text";
        $this->id_mk->Raw = true;
        $this->id_mk->IsAutoIncrement = true; // Autoincrement field
        $this->id_mk->IsPrimaryKey = true; // Primary key field
        $this->id_mk->Nullable = false; // NOT NULL field
        $this->id_mk->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_mk->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['id_mk'] = &$this->id_mk;
        ...
        // Dosen
        $this->Dosen = new DbField(
            $this, // Table
            'x_Dosen', // Variable name
            'Dosen', // Name
            '`Dosen`', // Expression
            '`Dosen`', // Basic search expression
            200, // Type
            255, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Dosen`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Dosen->InputTextType = "text";
        $this->Dosen->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['Dosen'] = &$this->Dosen;
    ...
    public function getDetailUrl(): string
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "detil_mata_kuliah") {
            $detailUrl = Container("detil_mata_kuliah")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_Kode_MK", $this->Kode_MK->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "matakuliahlist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "mata_kuliah";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_mk->setDbValue($conn->lastInsertId());
            $row['id_mk'] = $this->id_mk->DbValue;
        }
        return $result;
    }
    ...
    {
        // Cascade Update detail table 'detil_mata_kuliah'
        $cascadeUpdate = false;
        $rowCascade = [];
        if ($old && (isset($row['Kode_MK']) && $old['Kode_MK'] != $row['Kode_MK'])) { // Update detail field 'Kode_MK'
            $cascadeUpdate = true;
            $rowCascade['Kode_MK'] = $row['Kode_MK'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("detil_mata_kuliah")->loadRecords("`Kode_MK` = " . QuotedValue($old['Kode_MK'], DataType::STRING, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $oldDetail) {
                ...
                $success = Container("detil_mata_kuliah")->rowUpdating($oldDetail, $newDetail);
                if ($success) {
                    $success = Container("detil_mata_kuliah")->update($rowCascade, $detailKeys, $oldDetail);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("detil_mata_kuliah")->rowUpdated($oldDetail, $newDetail);
            }
        }
        ...
        if ($success) {
            if (!isset($row['id_mk']) && !IsEmpty($this->id_mk->CurrentValue)) {
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_mk', $row)) {
                AddFilter($where, QuotedName('id_mk', $this->Dbid) . '=' . QuotedValue($row['id_mk'], $this->id_mk->DataType, $this->Dbid));
            }
    ...
    public function delete(array $row, string|array $where = "", bool $currentFilter = false): int|bool
    {
        ...
        // Cascade delete detail table 'detil_mata_kuliah'
        $detailRows = Container("detil_mata_kuliah")->loadRecords("`Kode_MK` = " . QuotedValue($row['Kode_MK'], DataType::STRING, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($detailRows as $detailRow) {
            $success = Container("detil_mata_kuliah")->rowDeleting($detailRow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($detailRows as $detailRow) {
                $success = Container("detil_mata_kuliah")->delete($detailRow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($detailRows as $detailRow) {
                Container("detil_mata_kuliah")->rowDeleted($detailRow);
            }
        }
        if ($success) {
            try {
    ...
    {
        return "`id_mk` = @id_mk@ AND `Kode_MK` = '@Kode_MK@'";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_mk->CurrentValue : $this->id_mk->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 2) {
            if ($current) {
                $this->id_mk->CurrentValue = $keys[0];
            } else {
                $this->id_mk->OldValue = $keys[0];
            }
            if ($current) {
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_mk', $row) ? $row['id_mk'] : null;
        } else {
            $val = !IsEmpty($this->id_mk->OldValue) && !$current ? $this->id_mk->OldValue : $this->id_mk->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_mk@", AdjustSql($val), $keyFilter); // Replace key value
        }
        if (is_array($row)) {
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("matakuliahlist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "matakuliahview" => $this->language->phrase("View"),
            "matakuliahedit" => $this->language->phrase("Edit"),
            "matakuliahadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "MataKuliahView",
            Config("API_ADD_ACTION") => "MataKuliahAdd",
            Config("API_EDIT_ACTION") => "MataKuliahEdit",
            Config("API_DELETE_ACTION") => "MataKuliahDelete",
            Config("API_LIST_ACTION") => "MataKuliahList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("matakuliahview", $parm);
        } else {
            $url = $this->keyUrl("matakuliahview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("matakuliahedit", $parm);
        } else {
            $url = $this->keyUrl("matakuliahedit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("matakuliahlist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("matakuliahadd", $parm);
        } else {
            $url = $this->keyUrl("matakuliahadd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("matakuliahlist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("matakuliahdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_mk\":" . VarToJson($this->id_mk->CurrentValue, "number");
    ...
    {
        if ($this->id_mk->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_mk->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_mk") ?? Route("id_mk")) !== null) {
                $arKey[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_mk->CurrentValue = $key[0];
            } else {
                $this->id_mk->OldValue = $key[0];
            }
            if ($setCurrent) {
