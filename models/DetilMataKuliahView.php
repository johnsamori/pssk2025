 ...
 * Page class
 */
class DetilMataKuliahView extends DetilMataKuliah
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "DetilMataKuliahView";
    ...
    // CSS class/style
    public string $CurrentPageName = "detilmatakuliahview";
...
// Is modal
        ...
        if (($keyValue = Get("id_no") ?? Route("id_no")) !== null) {
            ...
            $this->RecKey["id_no"] = $this->id_no->QueryStringValue;
        } elseif (Post("id_no") !== null) {
            $this->id_no->setFormValue(Post("id_no"));
            $this->RecKey["id_no"] = $this->id_no->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->id_no->setQueryStringValue($keyValue);
            $this->RecKey["id_no"] = $this->id_no->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "detilmatakuliahlist"; // Return to list
        }
        if (($keyValue = Get("Kode_MK") ?? Route("Kode_MK")) !== null) {
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("detilmatakuliahlist"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->id_no->CurrentValue, $this->CurrentRow['id_no']) && SameString($this->Kode_MK->CurrentValue, $this->CurrentRow['Kode_MK'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "detilmatakuliahlist"; // No matching record, return to list
                    } else {
