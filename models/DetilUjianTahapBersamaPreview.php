 ...
 * Page class
 */
class DetilUjianTahapBersamaPreview extends DetilUjianTahapBersama
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "DetilUjianTahapBersamaPreview";
    ...
    // CSS class/style
    public string $CurrentPageName = "detilujiantahapbersamapreview";
...
// Use layout
    ...
    protected function masterKeyUrl(): string
    {
        ...
        if ($masterTblVar == "ujian_tahap_bersama") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=ujian_tahap_bersama&" . GetForeignKeyUrl("fk_id_utb", $this->id_utb->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys(array $keys): void
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "ujian_tahap_bersama") {
            $this->id_utb->setQueryStringValue($keys[0] ?? "");
        }
    }
