 ...
 * Page class
 */
class DetilMataKuliahGrid extends DetilMataKuliah
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "DetilMataKuliahGrid";
    ...
    public string $FormName = "fdetil_mata_kuliahgrid";

    // CSS class/style
    public string $CurrentPageName = "detilmatakuliahgrid";
...
// Use layout
    ...
    protected function setupListOptions(): void
    {
        ...
        $item->CssClass = "text-nowrap";
        $item->Visible = $this->security->allowView(CurrentProjectID() . 'detil_mata_kuliah');
        $item->OnLeft = true;

        // "edit"
        $item = &$this->ListOptions->add("edit");
        $item->CssClass = "text-nowrap";
        $item->Visible = $this->security->allowEdit(CurrentProjectID() . 'detil_mata_kuliah');
        $item->OnLeft = true;

        // "copy"
        $item = &$this->ListOptions->add("copy");
        $item->CssClass = "text-nowrap";
        $item->Visible = $this->security->allowAdd(CurrentProjectID() . 'detil_mata_kuliah');
        $item->OnLeft = true;

        // "delete"
        $item = &$this->ListOptions->add("delete");
        $item->CssClass = "text-nowrap";
        $item->Visible = $this->security->allowDelete(CurrentProjectID() . 'detil_mata_kuliah');
        $item->OnLeft = true;
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->allowView(CurrentProjectID() . 'detil_mata_kuliah')) {
            ...
            if ($this->security->allowEdit(CurrentProjectID() . 'detil_mata_kuliah')) {
            ...
            if ($this->security->allowAdd(CurrentProjectID() . 'detil_mata_kuliah')) {
            ...
            if ($this->security->allowDelete(CurrentProjectID() . 'detil_mata_kuliah')) {
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") { // Check view mode
            ...
            $item->Visible = $this->AddUrl != "" && $this->security->allowAdd(CurrentProjectID() . 'detil_mata_kuliah');
        }
    }
...
// Get upload files
    ...
    protected function addRow(?array $oldRow = null): bool
    {
        ...
        if ($this->getCurrentMasterTable() == "mata_kuliah") {
            $this->Kode_MK->Visible = true; // Need to insert foreign key
