 ...
 * Page class
 */
class UjianTahapBersamaView extends UjianTahapBersama
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "UjianTahapBersamaView";
    ...
    // CSS class/style
    public string $CurrentPageName = "ujiantahapbersamaview";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Set up record key
        if (($keyValue = Get("id_utb") ?? Route("id_utb")) !== null) {
            $this->RecKey["id_utb"] = $keyValue;
        }
...
// Is modal
        ...
        if (($keyValue = Get("id_utb") ?? Route("id_utb")) !== null) {
            $this->id_utb->setQueryStringValue($keyValue);
            $this->RecKey["id_utb"] = $this->id_utb->QueryStringValue;
        } elseif (Post("id_utb") !== null) {
            $this->id_utb->setFormValue(Post("id_utb"));
            $this->RecKey["id_utb"] = $this->id_utb->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->id_utb->setQueryStringValue($keyValue);
            $this->RecKey["id_utb"] = $this->id_utb->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "ujiantahapbersamalist"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("ujiantahapbersamalist"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->id_utb->CurrentValue, $this->CurrentRow['id_utb'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "ujiantahapbersamalist"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions(): void
    {
        ...
        */
        ...
        $item = &$option->add("detail_detil_ujian_tahap_bersama");
        $body = $this->language->phrase("ViewPageDetailLink") . $this->language->tablePhrase("detil_ujian_tahap_bersama", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("detilujiantahapbersamalist?" . Config("TABLE_SHOW_MASTER") . "=ujian_tahap_bersama&" . GetForeignKeyUrl("fk_id_utb", $this->id_utb->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'ujian_tahap_bersama')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($this->language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=detil_ujian_tahap_bersama"))) . "\">" . $this->language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'ujian_tahap_bersama')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($this->language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=detil_ujian_tahap_bersama"))) . "\">" . $this->language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'ujian_tahap_bersama')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($this->language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=detil_ujian_tahap_bersama"))) . "\">" . $this->language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
...
/**
    ...
    protected function setupDetailParms(): void
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("detil_ujian_tahap_bersama", $detailTblVar)) {
                $detailPageObj = Container("DetilUjianTahapBersamaGrid");
                if ($detailPageObj->DetailView) {
                    ...
                    $detailPageObj->id_utb->IsDetailKey = true;
                    $detailPageObj->id_utb->CurrentValue = $this->id_utb->CurrentValue;
                    $detailPageObj->id_utb->setSessionValue($detailPageObj->id_utb->CurrentValue);
                }
            }
        }
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ujiantahapbersamalist"), "", $this->TableVar, true);
        $pageId = "view";
