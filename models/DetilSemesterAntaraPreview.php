 ...
 * Page class
 */
class DetilSemesterAntaraPreview extends DetilSemesterAntara
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "DetilSemesterAntaraPreview";
    ...
    // CSS class/style
    public string $CurrentPageName = "detilsemesterantarapreview";
...
// Use layout
    ...
    protected function masterKeyUrl(): string
    {
        ...
        if ($masterTblVar == "semester_antara") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=semester_antara&" . GetForeignKeyUrl("fk_id_smtr", $this->id_smtsr->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys(array $keys): void
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "semester_antara") {
            $this->id_smtsr->setQueryStringValue($keys[0] ?? "");
        }
    }
