...
/**
 * Table class for beasiswa
 */
class Beasiswa extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $id_beasiswa;
    public DbField $Nama_Beasiswa;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "beasiswa"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_beasiswa
        $this->id_beasiswa = new DbField(
            $this, // Table
            'x_id_beasiswa', // Variable name
            'id_beasiswa', // Name
            '`id_beasiswa`', // Expression
            '`id_beasiswa`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_beasiswa`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_beasiswa->InputTextType = "text";
        $this->id_beasiswa->Raw = true;
        $this->id_beasiswa->IsAutoIncrement = true; // Autoincrement field
        $this->id_beasiswa->IsPrimaryKey = true; // Primary key field
        $this->id_beasiswa->Nullable = false; // NOT NULL field
        $this->id_beasiswa->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_beasiswa->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['id_beasiswa'] = &$this->id_beasiswa;

        // Nama_Beasiswa
        $this->Nama_Beasiswa = new DbField(
            $this, // Table
            'x_Nama_Beasiswa', // Variable name
            'Nama_Beasiswa', // Name
            '`Nama_Beasiswa`', // Expression
            '`Nama_Beasiswa`', // Basic search expression
            200, // Type
            50, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Nama_Beasiswa`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Nama_Beasiswa->InputTextType = "text";
        $this->Nama_Beasiswa->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['Nama_Beasiswa'] = &$this->Nama_Beasiswa;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "beasiswa";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_beasiswa->setDbValue($conn->lastInsertId());
            $row['id_beasiswa'] = $this->id_beasiswa->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_beasiswa']) && !IsEmpty($this->id_beasiswa->CurrentValue)) {
                $row['id_beasiswa'] = $this->id_beasiswa->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_beasiswa', $row)) {
                AddFilter($where, QuotedName('id_beasiswa', $this->Dbid) . '=' . QuotedValue($row['id_beasiswa'], $this->id_beasiswa->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->id_beasiswa->DbValue = $row['id_beasiswa'];
        $this->Nama_Beasiswa->DbValue = $row['Nama_Beasiswa'];
    }
    ...
    {
        return "`id_beasiswa` = @id_beasiswa@";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->id_beasiswa->CurrentValue : $this->id_beasiswa->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_beasiswa->CurrentValue = $keys[0];
            } else {
                $this->id_beasiswa->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_beasiswa', $row) ? $row['id_beasiswa'] : null;
        } else {
            $val = !IsEmpty($this->id_beasiswa->OldValue) && !$current ? $this->id_beasiswa->OldValue : $this->id_beasiswa->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_beasiswa@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("beasiswalist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "beasiswaview" => $this->language->phrase("View"),
            "beasiswaedit" => $this->language->phrase("Edit"),
            "beasiswaadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "BeasiswaView",
            Config("API_ADD_ACTION") => "BeasiswaAdd",
            Config("API_EDIT_ACTION") => "BeasiswaEdit",
            Config("API_DELETE_ACTION") => "BeasiswaDelete",
            Config("API_LIST_ACTION") => "BeasiswaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("beasiswaview", $parm);
        } else {
            $url = $this->keyUrl("beasiswaview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("beasiswaedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("beasiswalist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("beasiswaadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("beasiswalist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("beasiswadelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_beasiswa\":" . VarToJson($this->id_beasiswa->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_beasiswa->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_beasiswa->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_beasiswa") ?? Route("id_beasiswa")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_beasiswa->CurrentValue = $key;
            } else {
                $this->id_beasiswa->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->id_beasiswa);
                    $doc->exportCaption($this->Nama_Beasiswa);
                } else {
                    $doc->exportCaption($this->id_beasiswa);
                    $doc->exportCaption($this->Nama_Beasiswa);
                }
        ...
        while (($row = $result->fetchAssociative()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->id_beasiswa);
                        $doc->exportField($this->Nama_Beasiswa);
                    } else {
                        $doc->exportField($this->id_beasiswa);
                        $doc->exportField($this->Nama_Beasiswa);
                    }
    ...
    public function renderLookupForView(string $name, mixed $value): mixed
    {
        ...
        if ($name == "Nama_Beasiswa") {
            $clone = $this->Nama_Beasiswa->getClone()->setViewValue($value);
