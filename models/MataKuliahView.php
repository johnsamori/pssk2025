 ...
 * Page class
 */
class MataKuliahView extends MataKuliah
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "MataKuliahView";
    ...
    // CSS class/style
    public string $CurrentPageName = "matakuliahview";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        if (($keyValue = Get("id_mk") ?? Route("id_mk")) !== null) {
            $this->RecKey["id_mk"] = $keyValue;
        }
        if (($keyValue = Get("Kode_MK") ?? Route("Kode_MK")) !== null) {
            $this->RecKey["Kode_MK"] = $keyValue;
        }
...
// Is modal
        ...
        if (($keyValue = Get("id_mk") ?? Route("id_mk")) !== null) {
            ...
            $this->RecKey["id_mk"] = $this->id_mk->QueryStringValue;
        } elseif (Post("id_mk") !== null) {
            $this->id_mk->setFormValue(Post("id_mk"));
            $this->RecKey["id_mk"] = $this->id_mk->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->id_mk->setQueryStringValue($keyValue);
            $this->RecKey["id_mk"] = $this->id_mk->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "matakuliahlist"; // Return to list
        }
        if (($keyValue = Get("Kode_MK") ?? Route("Kode_MK")) !== null) {
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("matakuliahlist"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->id_mk->CurrentValue, $this->CurrentRow['id_mk']) && SameString($this->Kode_MK->CurrentValue, $this->CurrentRow['Kode_MK'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "matakuliahlist"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions(): void
    {
        ...
        */
        ...
        $item = &$option->add("detail_detil_mata_kuliah");
        $body = $this->language->phrase("ViewPageDetailLink") . $this->language->tablePhrase("detil_mata_kuliah", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("detilmatakuliahlist?" . Config("TABLE_SHOW_MASTER") . "=mata_kuliah&" . GetForeignKeyUrl("fk_Kode_MK", $this->Kode_MK->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'mata_kuliah')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($this->language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=detil_mata_kuliah"))) . "\">" . $this->language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'mata_kuliah')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($this->language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=detil_mata_kuliah"))) . "\">" . $this->language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'mata_kuliah')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($this->language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=detil_mata_kuliah"))) . "\">" . $this->language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
