 ...
 * Page class
 */
class MataKuliahList extends MataKuliah
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "MataKuliahList";
    ...
    public string $FormName = "fmata_kuliahlist";

    // CSS class/style
    public string $CurrentPageName = "matakuliahlist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Initialize URLs
        $this->AddUrl = "matakuliahadd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "matakuliahdelete";
        $this->MultiUpdateUrl = "matakuliahupdate";
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "fmata_kuliahgrid";
        }
		...
		if (ReadCookie('mata_kuliah_searchpanel') == 'notactive' || ReadCookie('mata_kuliah_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('mata_kuliah_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fmata_kuliahsrch");
        }
        $filterList = Concat($filterList, $this->id_mk->AdvancedSearch->toJson(), ","); // Field id_mk
        ...
        $filterList = Concat($filterList, $this->Dosen->AdvancedSearch->toJson(), ","); // Field Dosen
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fmata_kuliahsrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field id_mk
        $this->id_mk->AdvancedSearch->SearchValue = $filter["x_id_mk"] ?? "";
        $this->id_mk->AdvancedSearch->SearchOperator = $filter["z_id_mk"] ?? "";
        $this->id_mk->AdvancedSearch->SearchCondition = $filter["v_id_mk"] ?? "";
        $this->id_mk->AdvancedSearch->SearchValue2 = $filter["y_id_mk"] ?? "";
        $this->id_mk->AdvancedSearch->SearchOperator2 = $filter["w_id_mk"] ?? "";
        $this->id_mk->AdvancedSearch->save();
        ...
        // Field Dosen
        $this->Dosen->AdvancedSearch->SearchValue = $filter["x_Dosen"] ?? "";
        $this->Dosen->AdvancedSearch->SearchOperator = $filter["z_Dosen"] ?? "";
        $this->Dosen->AdvancedSearch->SearchCondition = $filter["v_Dosen"] ?? "";
        $this->Dosen->AdvancedSearch->SearchValue2 = $filter["y_Dosen"] ?? "";
        $this->Dosen->AdvancedSearch->SearchOperator2 = $filter["w_Dosen"] ?? "";
        $this->Dosen->AdvancedSearch->save();
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_mk); // id_mk
            ...
            $this->updateSort($this->Dosen); // Dosen
    ...
    protected function setupListOptions(): void
    {
        ...
        // "detail_detil_mata_kuliah"
        $item = &$this->ListOptions->add("detail_detil_mata_kuliah");
        $item->CssClass = "text-nowrap";
        $item->Visible = $this->security->allowList(CurrentProjectID() . 'detil_mata_kuliah');
        $item->OnLeft = true;
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"mata_kuliah\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"mata_kuliah\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"mata_kuliah\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fmata_kuliahlist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fmata_kuliahlist\" data-key=\"" . $this->keyToJson(true) .
        ...
        // "detail_detil_mata_kuliah"
        $opt = $this->ListOptions["detail_detil_mata_kuliah"];
        if ($this->security->allowList(CurrentProjectID() . 'detil_mata_kuliah')) {
            $body = $this->language->phrase("DetailLink") . $this->language->tablePhrase("detil_mata_kuliah", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("detilmatakuliahlist?" . Config("TABLE_SHOW_MASTER") . "=mata_kuliah&" . GetForeignKeyUrl("fk_Kode_MK", $this->Kode_MK->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("DetilMataKuliahGrid");
            if ($detailPage->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'mata_kuliah')) {
                $caption = $this->language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=detil_mata_kuliah");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "detil_mata_kuliah";
            }
            if ($detailPage->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'mata_kuliah')) {
                $caption = $this->language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=detil_mata_kuliah");
                ...
                $detailEditTblVar .= "detil_mata_kuliah";
            }
            if ($detailPage->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'mata_kuliah')) {
                $caption = $this->language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=detil_mata_kuliah");
                ...
                $detailCopyTblVar .= "detil_mata_kuliah";
            }
            if ($links != "") {
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_mk->CurrentValue . Config("COMPOSITE_KEY_SEPARATOR") . $this->Kode_MK->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
        ...
        $detailFilters = [];
        $masterKeys = []; // Reset
        $masterKeys["Kode_MK"] = strval($this->Kode_MK->DbValue);

        // Column "detail_detil_mata_kuliah"
        if ($this->DetailPages?->getItem("detil_mata_kuliah")?->Visible && $this->security->allowList(CurrentProjectID() . 'detil_mata_kuliah')) {
            $link = "";
            $option = $this->ListOptions["detail_detil_mata_kuliah"];
            $detailTbl = Container("detil_mata_kuliah");
            ...
            $url = "detilmatakuliahpreview?t=mata_kuliah&f=" . Encrypt($detailFilter . "|" . implode("|", array_values($masterKeys)));
            $btngrp = "<div data-table=\"detil_mata_kuliah\" data-url=\"" . $url . "\" class=\"ew-detail-btn-group btn-group btn-group-sm d-none\">";
            if ($this->security->allowList(CurrentProjectID() . 'mata_kuliah')) {
                $label = $this->language->tablePhrase("detil_mata_kuliah", "TblCaption");

        		// Begin of modification by Masino Sinaga, December 11, 2024
                // $link = "<button class=\"nav-link\" data-bs-toggle=\"tab\" data-table=\"detil_mata_kuliah\" data-url=\"" . $url . "\" type=\"button\" role=\"tab\" aria-selected=\"false\">" . $label . "</button>";
                ...
                $detaillnk = GetUrl("detilmatakuliahlist?" . Config("TABLE_SHOW_MASTER") . "=mata_kuliah");
                foreach ($masterKeys as $key => $value) {
                    $detaillnk .= "&" . GetForeignKeyUrl("fk_$key", $value);
                }
                $title = $this->language->tablePhrase("detil_mata_kuliah", "TblCaption");
            ...
            if ($detailPageObj->DetailView && $this->security->canView() && $this->security->allowView(CurrentProjectID() . 'mata_kuliah')) {
                $caption = $this->language->phrase("MasterDetailViewLink");
                $viewurl = GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=detil_mata_kuliah"));
            ...
            if ($detailPageObj->DetailEdit && $this->security->canEdit() && $this->security->allowEdit(CurrentProjectID() . 'mata_kuliah')) {
                $caption = $this->language->phrase("MasterDetailEditLink");
                $editurl = GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=detil_mata_kuliah"));
            ...
            if ($detailPageObj->DetailAdd && $this->security->canAdd() && $this->security->allowAdd(CurrentProjectID() . 'mata_kuliah')) {
                $caption = $this->language->phrase("MasterDetailCopyLink");
                $copyurl = GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=detil_mata_kuliah"));
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"mata_kuliah\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        $detailTableLink = "";
        $item = &$option->add("detailadd_detil_mata_kuliah");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=detil_mata_kuliah");
        $detailPage = Container("DetilMataKuliahGrid");
        $caption = $this->language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $this->security->allowAdd(CurrentProjectID() . 'mata_kuliah') && $this->security->canAdd());
        if ($item->Visible) {
            if ($detailTableLink != "") {
                $detailTableLink .= ",";
            }
            $detailTableLink .= "detil_mata_kuliah";
        }
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "id_mk");
            ...
            $this->createColumnOption($option, "Dosen");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fmata_kuliahsrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fmata_kuliahsrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fmata_kuliahlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_mata_kuliah", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_mata_kuliah",
...
// Load basic search values
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('mata_kuliah_searchpanel') == 'notactive' || ReadCookie('mata_kuliah_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmata_kuliahsrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('mata_kuliah_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmata_kuliahsrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fmata_kuliahsrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
