 ...
 * Page class
 */
class DetilUjianTahapBersamaList extends DetilUjianTahapBersama
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "DetilUjianTahapBersamaList";
    ...
    public string $FormName = "fdetil_ujian_tahap_bersamalist";

    // CSS class/style
    public string $CurrentPageName = "detilujiantahapbersamalist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->TableVar = 'detil_ujian_tahap_bersama';
        $this->TableName = 'detil_ujian_tahap_bersama';
        ...
        // Table object (detil_ujian_tahap_bersama)
        if (!isset($GLOBALS["detil_ujian_tahap_bersama"]) || $GLOBALS["detil_ujian_tahap_bersama"]::class == PROJECT_NAMESPACE . "detil_ujian_tahap_bersama") {
            $GLOBALS["detil_ujian_tahap_bersama"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "detilujiantahapbersamaadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "detilujiantahapbersamadelete";
        $this->MultiUpdateUrl = "detilujiantahapbersamaupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'detil_ujian_tahap_bersama');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "detilujiantahapbersamaview"); // If View page, no primary button
                } else { // List page
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "fdetil_ujian_tahap_bersamagrid";
        }
        ...
        if (IsPost() && ($this->isGridUpdate() || $this->isMultiUpdate() || $this->isGridOverwrite()) && (Session(AddTabId(SESSION_INLINE_MODE)) == "gridedit" || Session(AddTabId(SESSION_INLINE_MODE)) == "multiedit")) {
            ...
            if ($gridUpdate) {
                ...
                if ($this->IsModal && !$this->UseAjaxActions) {
                    $this->terminate("detilujiantahapbersamalist");
                    return;
                }
            } else {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "ujian_tahap_bersama") {
            $masterTbl = Container("ujian_tahap_bersama");
            ...
            if (!$this->MasterRecordExists) {
                ...
                $this->terminate("ujiantahapbersamalist"); // Return to master page
                return;
            } else {
		...
		if (ReadCookie('detil_ujian_tahap_bersama_searchpanel') == 'notactive' || ReadCookie('detil_ujian_tahap_bersama_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('detil_ujian_tahap_bersama_searchpanel') == 'active') {
    ...
    {
        if (
            $this->hasFormValue("x_id_utb")
            && $this->hasFormValue("o_id_utb")
            && $this->id_utb->CurrentValue != $this->id_utb->DefaultValue
            && !($this->id_utb->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->id_utb->CurrentValue == $this->id_utb->getSessionValue())
        ) {
            return false;
        }
        if (
        ...
        if (
            $this->hasFormValue("x_Nilai")
            && $this->hasFormValue("o_Nilai")
            && $this->Nilai->CurrentValue != $this->Nilai->DefaultValue
            && !($this->Nilai->IsForeignKey && $this->getCurrentMasterTable() != "" && $this->Nilai->CurrentValue == $this->Nilai->getSessionValue())
        ) {
            return false;
        }
        return true;
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fdetil_ujian_tahap_bersamasrch");
        }
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fdetil_ujian_tahap_bersamasrch", $filters);
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"detil_ujian_tahap_bersama\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"detil_ujian_tahap_bersama\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"detil_ujian_tahap_bersama\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdetil_ujian_tahap_bersamalist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdetil_ujian_tahap_bersamalist\" data-key=\"" . $this->keyToJson(true) .
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"detil_ujian_tahap_bersama\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdetil_ujian_tahap_bersamasrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdetil_ujian_tahap_bersamasrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit() && !$this->isMultiEdit()) { // Not grid add/grid edit/multi edit mode
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
                if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                    ...
                    $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdetil_ujian_tahap_bersamalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                    $item->Visible = true;
                }
            }
            ...
            if ($this->isGridAdd()) {
                ...
                if (!($this->ModalGridAdd && !IsMobile())) {
                    ...
                    $item->Body = "<button class=\"ew-action ew-grid-insert\" title=\"" . HtmlTitle($this->language->phrase("GridInsertLink")) . "\" data-caption=\"" . HtmlTitle($this->language->phrase("GridInsertLink")) . "\" form=\"fdetil_ujian_tahap_bersamalist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $this->language->phrase("GridInsertLink") . "</button>";
                    // Add grid cancel
            ...
            if ($this->isGridEdit()) {
                ...
                if (!($this->ModalGridEdit && !IsMobile())) {
                    ...
                    $option->UseDropDownButton = false;
                        ...
                        $item->Body = "<button class=\"ew-action ew-grid-save\" title=\"" . HtmlTitle($this->language->phrase("GridSaveLink")) . "\" data-caption=\"" . HtmlTitle($this->language->phrase("GridSaveLink")) . "\" form=\"fdetil_ujian_tahap_bersamalist\" formaction=\"" . GetUrl($this->pageName()) . "\">" . $this->language->phrase("GridSaveLink") . "</button>";
                        $item = &$option->add("gridcancel");
                        $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_detil_ujian_tahap_bersama", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_detil_ujian_tahap_bersama",
...
// Load default values
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'id_utb' before field var 'x_id_utb'
        $val = $this->getFormValue("id_utb", null) ?? $this->getFormValue("x_id_utb", null);
        if (!$this->id_utb->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->id_utb->Visible = false; // Disable update for API request
            } else {
                $this->id_utb->setFormValue($val);
            }
        }
        if ($this->hasFormValue("o_id_utb")) {
            $this->id_utb->setOldValue($this->getFormValue("o_id_utb"));
        }
        ...
        // Check field name 'Nilai' before field var 'x_Nilai'
        $val = $this->getFormValue("Nilai", null) ?? $this->getFormValue("x_Nilai", null);
        ...
        if ($this->hasFormValue("o_Nilai")) {
            $this->Nilai->setOldValue($this->getFormValue("o_Nilai"));
        }
    }
    ...
    public function restoreFormValues(): void
    {
        ...
        $this->id_utb->CurrentValue = $this->id_utb->FormValue;
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // id_utb
            $this->id_utb->setupEditAttributes();
            if ($this->id_utb->getSessionValue() != "") {
                $this->id_utb->CurrentValue = GetForeignKeyValue($this->id_utb->getSessionValue());
                $this->id_utb->OldValue = $this->id_utb->CurrentValue;
                $this->id_utb->ViewValue = $this->id_utb->CurrentValue;
            } else {
                $this->id_utb->EditValue = !$this->id_utb->Raw ? HtmlDecode($this->id_utb->CurrentValue) : $this->id_utb->CurrentValue;
                $this->id_utb->PlaceHolder = RemoveHtml($this->id_utb->caption());
            }
            ...
            $this->Nilai->EditValue = !$this->Nilai->Raw ? HtmlDecode($this->Nilai->CurrentValue) : $this->Nilai->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            ...
            if ($this->id_utb->Visible && $this->id_utb->Required) {
                if (!$this->id_utb->IsDetailKey && IsEmpty($this->id_utb->FormValue)) {
                    $this->id_utb->addErrorMessage(str_replace("%s", $this->id_utb->caption(), $this->id_utb->RequiredErrorMessage));
                }
            }
            ...
            if ($this->Nilai->Visible && $this->Nilai->Required) {
                if (!$this->Nilai->IsDetailKey && IsEmpty($this->Nilai->FormValue)) {
    ...
    protected function editRow(): bool
    {
        ...
        // Check referential integrity for master table 'ujian_tahap_bersama'
        $detailKeys = [];
        $keyValue = $newRow['id_utb'] ?? $oldRow['id_utb'];
        $detailKeys['id_utb'] = $keyValue;
        $masterTable = Container("ujian_tahap_bersama");
        ...
        if (!$validMasterRecord) {
            $relatedRecordMsg = sprintf($this->language->phrase("RelatedRecordRequired"), "ujian_tahap_bersama");
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // id_utb
        if ($this->id_utb->getSessionValue() != "") {
            $this->id_utb->ReadOnly = true;
        }
        $this->id_utb->setDbValueDef($newRow, $this->id_utb->CurrentValue, $this->id_utb->ReadOnly);

        // NIM
        $this->NIM->setDbValueDef($newRow, $this->NIM->CurrentValue, $this->NIM->ReadOnly);

        // Nilai
        $this->Nilai->setDbValueDef($newRow, $this->Nilai->CurrentValue, $this->Nilai->ReadOnly);
        return $newRow;
    }
    ...
    {
        if (isset($row['id_utb'])) { // id_utb
            $this->id_utb->CurrentValue = $row['id_utb'];
        }
    ...
    public function getRowHash(array $row): ?string
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['id_utb']); // id_utb
        $hash .= GetFieldHash($row['NIM']); // NIM
        $hash .= GetFieldHash($row['Nilai']); // Nilai
        return md5($hash);
    }
    ...
    protected function addRow(?array $oldRow = null): bool
    {
        ...
        // Check referential integrity for master table 'detil_ujian_tahap_bersama'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["id_utb"] = $this->id_utb->CurrentValue;
        $masterTable = Container("ujian_tahap_bersama");
        $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
        if (!IsEmpty($masterFilter)) {
            $masterRow = $masterTable->loadRecords($masterFilter)->fetchAssociative();
    ...
    protected function getAddRow(): array
    {
        ...
        // id_utb
        $this->id_utb->setDbValueDef($newRow, $this->id_utb->CurrentValue, false);

        // NIM
        $this->NIM->setDbValueDef($newRow, $this->NIM->CurrentValue, false);

        // Nilai
        $this->Nilai->setDbValueDef($newRow, $this->Nilai->CurrentValue, false);
        return $newRow;
    }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('detil_ujian_tahap_bersama_searchpanel') == 'notactive' || ReadCookie('detil_ujian_tahap_bersama_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdetil_ujian_tahap_bersamasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('detil_ujian_tahap_bersama_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdetil_ujian_tahap_bersamasrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdetil_ujian_tahap_bersamasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
    ...
    protected function setupMasterParms(): void
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "ujian_tahap_bersama") {
                $validMaster = true;
                $masterTbl = Container("ujian_tahap_bersama");
                if (($parm = Get("fk_id_utb", Get("id_utb"))) !== null) {
                    $masterTbl->id_utb->setQueryStringValue($parm);
                    $this->id_utb->QueryStringValue = $masterTbl->id_utb->QueryStringValue; // DO NOT change, master/detail key data type can be different
                    $this->id_utb->setSessionValue($this->id_utb->QueryStringValue);
                    $foreignKeys["id_utb"] = $this->id_utb->QueryStringValue;
                } else {
                    $validMaster = false;
                }
            }
            ...
            if ($masterTblVar == "ujian_tahap_bersama") {
                $validMaster = true;
                $masterTbl = Container("ujian_tahap_bersama");
                if (($parm = Post("fk_id_utb", Post("id_utb"))) !== null) {
                    $masterTbl->id_utb->setFormValue($parm);
                    $this->id_utb->FormValue = $masterTbl->id_utb->FormValue;
                    $this->id_utb->setSessionValue($this->id_utb->FormValue);
                    $foreignKeys["id_utb"] = $this->id_utb->FormValue;
                } else {
                    $validMaster = false;
                }
            }
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "ujian_tahap_bersama") {
                if (!array_key_exists("id_utb", $foreignKeys)) { // Not current foreign key
                    $this->id_utb->setSessionValue("");
                }
            }
        }
