 ...
 * Page class
 */
class DaftarMatakuliahEdit extends DaftarMatakuliah
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "DaftarMatakuliahEdit";
    ...
    // CSS class/style
    public string $CurrentPageName = "daftarmatakuliahedit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("daftarmatakuliahlist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "daftarmatakuliahlist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "daftarmatakuliahlist") {
                            ...
                            $returnUrl = "daftarmatakuliahlist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
...
// Get upload files
    ...
    protected function loadFormValues(): void
    {
        ...
        // Check field name 'Kode' before field var 'x_Kode'
        $val = $this->getFormValue("Kode", null) ?? $this->getFormValue("x_Kode", null);
        ...
        if ($this->hasFormValue("o_Kode")) {
            $this->Kode->setOldValue($this->getFormValue("o_Kode"));
        }
        ...
        // Check field name 'SKS' before field var 'x_SKS'
        $val = $this->getFormValue("SKS", null) ?? $this->getFormValue("x_SKS", null);
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->Kode->EditValue = !$this->Kode->Raw ? HtmlDecode($this->Kode->CurrentValue) : $this->Kode->CurrentValue;
            ...
            // SKS
            $this->SKS->setupEditAttributes();
            $this->SKS->EditValue = !$this->SKS->Raw ? HtmlDecode($this->SKS->CurrentValue) : $this->SKS->CurrentValue;
    ...
    protected function validateForm(): bool
    {
        ...
        $validateForm = true;
            if ($this->Kode->Visible && $this->Kode->Required) {
                if (!$this->Kode->IsDetailKey && IsEmpty($this->Kode->FormValue)) {
            ...
            if ($this->SKS->Visible && $this->SKS->Required) {
                if (!$this->SKS->IsDetailKey && IsEmpty($this->SKS->FormValue)) {
                    $this->SKS->addErrorMessage(str_replace("%s", $this->SKS->caption(), $this->SKS->RequiredErrorMessage));
                }
            }
    ...
    protected function editRow(): bool
    {
        ...
        // Check field with unique index (Kode)
        if ($this->Kode->CurrentValue != "") {
            $filterChk = "(`Kode` = '" . AdjustSql($this->Kode->CurrentValue) . "')";
            ...
            if ($rsChk->fetchAssociative()) {
                $idxErrMsg = sprintf($this->language->phrase("DuplicateIndex"), $this->Kode->CurrentValue, $this->Kode->caption());
    ...
    protected function getEditRow(array $oldRow): array
    {
        ...
        // Kode
        $this->Kode->setDbValueDef($newRow, $this->Kode->CurrentValue, $this->Kode->ReadOnly);

        // Nama
        $this->Nama->setDbValueDef($newRow, $this->Nama->CurrentValue, $this->Nama->ReadOnly);

        // SKS
        $this->SKS->setDbValueDef($newRow, $this->SKS->CurrentValue, $this->SKS->ReadOnly);
        return $newRow;
    }
    ...
    protected function restoreEditFormFromRow(array $row): void
    {
        ...
        if (isset($row['SKS'])) { // SKS
            $this->SKS->CurrentValue = $row['SKS'];
        }
    }
    ...
    protected function setupBreadcrumb(): void
    {
        ...
        $url = CurrentUrl();
        $breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("daftarmatakuliahlist"), "", $this->TableVar, true);
        $pageId = "edit";
