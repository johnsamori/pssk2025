...
/**
 * Table class for tahun_akademik
 */
class TahunAkademik extends DbTable implements LookupTableInterface
{
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "tahun_akademik"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_tahun
        $this->id_tahun = new DbField(
            $this, // Table
            'x_id_tahun', // Variable name
            'id_tahun', // Name
            '`id_tahun`', // Expression
            '`id_tahun`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_tahun`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_tahun->InputTextType = "text";
        $this->id_tahun->Raw = true;
        $this->id_tahun->IsAutoIncrement = true; // Autoincrement field
        $this->id_tahun->IsPrimaryKey = true; // Primary key field
        $this->id_tahun->Nullable = false; // NOT NULL field
        $this->id_tahun->DefaultErrorMessage = $this->language->phrase("IncorrectInteger");
        $this->id_tahun->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['id_tahun'] = &$this->id_tahun;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "tahun_akademik";
    }
    ...
    public function insert(array &$row): int|bool
    {
        ...
        if ($result) {
            $this->id_tahun->setDbValue($conn->lastInsertId());
            $row['id_tahun'] = $this->id_tahun->DbValue;
        }
        return $result;
    }
    ...
    public function update(array $row, string|array $where = "", ?array $old = null, bool $currentFilter = true): int|bool
    {
        ...
        if ($success) {
            if (!isset($row['id_tahun']) && !IsEmpty($this->id_tahun->CurrentValue)) {
                $row['id_tahun'] = $this->id_tahun->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_tahun', $row)) {
                AddFilter($where, QuotedName('id_tahun', $this->Dbid) . '=' . QuotedValue($row['id_tahun'], $this->id_tahun->DataType, $this->Dbid));
            }
        }
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->id_tahun->DbValue = $row['id_tahun'];
    ...
    {
        $keys = [];
        $val = $current ? $this->id_tahun->CurrentValue : $this->id_tahun->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_tahun->CurrentValue = $keys[0];
            } else {
                $this->id_tahun->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_tahun', $row) ? $row['id_tahun'] : null;
        } else {
            $val = !IsEmpty($this->id_tahun->OldValue) && !$current ? $this->id_tahun->OldValue : $this->id_tahun->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@id_tahun@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("tahunakademiklist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "tahunakademikview" => $this->language->phrase("View"),
            "tahunakademikedit" => $this->language->phrase("Edit"),
            "tahunakademikadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "TahunAkademikView",
            Config("API_ADD_ACTION") => "TahunAkademikAdd",
            Config("API_EDIT_ACTION") => "TahunAkademikEdit",
            Config("API_DELETE_ACTION") => "TahunAkademikDelete",
            Config("API_LIST_ACTION") => "TahunAkademikList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("tahunakademikview", $parm);
        } else {
            $url = $this->keyUrl("tahunakademikview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "tahunakademikadd?" . $parm;
        } else {
            $url = "tahunakademikadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("tahunakademikedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("tahunakademiklist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("tahunakademikadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("tahunakademiklist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("tahunakademikdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_tahun\":" . VarToJson($this->id_tahun->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_tahun->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_tahun->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_tahun") ?? Route("id_tahun")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_tahun->CurrentValue = $key;
            } else {
                $this->id_tahun->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->id_tahun);
