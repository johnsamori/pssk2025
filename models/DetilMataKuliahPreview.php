 ...
 * Page class
 */
class DetilMataKuliahPreview extends DetilMataKuliah
{
    use MessagesTrait;
    ...
    // Page object name
    public string $PageObjName = "DetilMataKuliahPreview";
    ...
    // CSS class/style
    public string $CurrentPageName = "detilmatakuliahpreview";
...
// Use layout
    ...
    protected function masterKeyUrl(): string
    {
        ...
        if ($masterTblVar == "mata_kuliah") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=mata_kuliah&" . GetForeignKeyUrl("fk_Kode_MK", $this->Kode_MK->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys(array $keys): void
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "mata_kuliah") {
            $this->Kode_MK->setQueryStringValue($keys[0] ?? "");
        }
    }
