...
/**
 * Table class for semester_antara
 */
class SemesterAntara extends DbTable implements LookupTableInterface
{
    ...
    // Fields
    public DbField $id_smtr;
    public DbField $Semester;
    public DbField $Jadwal;
    public DbField $Tahun_Akademik;
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        $this->UpdateTable = "semester_antara"; // Update table
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // id_smtr
        $this->id_smtr = new DbField(
            $this, // Table
            'x_id_smtr', // Variable name
            'id_smtr', // Name
            '`id_smtr`', // Expression
            '`id_smtr`', // Basic search expression
            200, // Type
            20, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`id_smtr`', // Virtual expression
            false, // Is virtual
        ...
        $this->id_smtr->InputTextType = "text";
        $this->id_smtr->Raw = true;
        $this->id_smtr->IsPrimaryKey = true; // Primary key field
        $this->id_smtr->IsForeignKey = true; // Foreign key field
        $this->id_smtr->Nullable = false; // NOT NULL field
        $this->id_smtr->Required = true; // Required field
        $this->id_smtr->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['id_smtr'] = &$this->id_smtr;

        // Semester
        $this->Semester = new DbField(
            $this, // Table
            'x_Semester', // Variable name
            'Semester', // Name
            '`Semester`', // Expression
            '`Semester`', // Basic search expression
            200, // Type
            50, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Semester`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Semester->InputTextType = "text";
        $this->Semester->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['Semester'] = &$this->Semester;

        // Jadwal
        $this->Jadwal = new DbField(
            $this, // Table
            'x_Jadwal', // Variable name
            'Jadwal', // Name
            '`Jadwal`', // Expression
            '`Jadwal`', // Basic search expression
            200, // Type
            255, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Jadwal`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Jadwal->InputTextType = "text";
        $this->Jadwal->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['Jadwal'] = &$this->Jadwal;

        // Tahun_Akademik
        $this->Tahun_Akademik = new DbField(
            $this, // Table
            'x_Tahun_Akademik', // Variable name
            'Tahun_Akademik', // Name
            '`Tahun_Akademik`', // Expression
            '`Tahun_Akademik`', // Basic search expression
            200, // Type
            50, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`Tahun_Akademik`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->Tahun_Akademik->InputTextType = "text";
        $this->Tahun_Akademik->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
    ...
    public function getDetailUrl(): string
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "detil_semester_antara") {
            $detailUrl = Container("detil_semester_antara")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id_smtr", $this->id_smtr->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "semesterantaralist";
        }
        return $detailUrl;
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "semester_antara";
    }
    ...
    {
        // Cascade Update detail table 'detil_semester_antara'
        $cascadeUpdate = false;
        $rowCascade = [];
        if ($old && (isset($row['id_smtr']) && $old['id_smtr'] != $row['id_smtr'])) { // Update detail field 'id_smtsr'
            $cascadeUpdate = true;
            $rowCascade['id_smtsr'] = $row['id_smtr'];
        }
        if ($cascadeUpdate) {
            $rswrk = Container("detil_semester_antara")->loadRecords("`id_smtsr` = " . QuotedValue($old['id_smtr'], DataType::STRING, 'DB'))->fetchAllAssociative();
            foreach ($rswrk as $oldDetail) {
                $detailKeys = [];
                $fldname = 'id_smtsr';
                $detailKeys[$fldname] = $oldDetail[$fldname];
                $fldname = 'no';
                $detailKeys[$fldname] = $oldDetail[$fldname];
                $newDetail = array_merge($oldDetail, $rowCascade);
                // Call Row_Updating event
                $success = Container("detil_semester_antara")->rowUpdating($oldDetail, $newDetail);
                if ($success) {
                    $success = Container("detil_semester_antara")->update($rowCascade, $detailKeys, $oldDetail);
                }
                if (!$success) {
                    return false;
                }
                // Call Row_Updated event
                Container("detil_semester_antara")->rowUpdated($oldDetail, $newDetail);
            }
        }
    ...
    public function deleteSql(?array $row, string|array $where = ""): QueryBuilder
    {
        ...
        $where = is_array($where) ? $this->arrayToFilter($where) : $where;
        if ($row) {
            if (array_key_exists('id_smtr', $row)) {
                AddFilter($where, QuotedName('id_smtr', $this->Dbid) . '=' . QuotedValue($row['id_smtr'], $this->id_smtr->DataType, $this->Dbid));
            }
        }
    ...
    public function delete(array $row, string|array $where = "", bool $currentFilter = false): int|bool
    {
        ...
        // Cascade delete detail table 'detil_semester_antara'
        $detailRows = Container("detil_semester_antara")->loadRecords("`id_smtsr` = " . QuotedValue($row['id_smtr'], DataType::STRING, "DB"))->fetchAllAssociative();
        // Call Row Deleting event
        foreach ($detailRows as $detailRow) {
            $success = Container("detil_semester_antara")->rowDeleting($detailRow);
            if (!$success) {
                break;
            }
        }
        if ($success) {
            foreach ($detailRows as $detailRow) {
                $success = Container("detil_semester_antara")->delete($detailRow); // Delete
                if (!$success) {
                    break;
                }
            }
        ...
        if ($success) {
            foreach ($detailRows as $detailRow) {
                Container("detil_semester_antara")->rowDeleted($detailRow);
            }
        }
        if ($success) {
            try {
    ...
    protected function loadDbValues(?array $row)
    {
        ...
        $this->id_smtr->DbValue = $row['id_smtr'];
        $this->Semester->DbValue = $row['Semester'];
        $this->Jadwal->DbValue = $row['Jadwal'];
    ...
    {
        $keys = [];
        $val = $current ? $this->id_smtr->CurrentValue : $this->id_smtr->OldValue;
        if (IsEmpty($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey(string $key, bool $current = false, ?string $keySeparator = null): void
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->id_smtr->CurrentValue = $keys[0];
            } else {
                $this->id_smtr->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter(?array $row = null, bool $current = false): string
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('id_smtr', $row) ? $row['id_smtr'] : null;
        } else {
            $val = !IsEmpty($this->id_smtr->OldValue) && !$current ? $this->id_smtr->OldValue : $this->id_smtr->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@id_smtr@", AdjustSql($val), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl(): string
    {
        ...
        return Session($name) ?? GetUrl("semesterantaralist");
    }
    ...
    public function getModalCaption(string $pageName): string
    {
        return match ($pageName) {
            "semesterantaraview" => $this->language->phrase("View"),
            "semesterantaraedit" => $this->language->phrase("Edit"),
            "semesterantaraadd" => $this->language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName(string $action): string
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "SemesterAntaraView",
            Config("API_ADD_ACTION") => "SemesterAntaraAdd",
            Config("API_EDIT_ACTION") => "SemesterAntaraEdit",
            Config("API_DELETE_ACTION") => "SemesterAntaraDelete",
            Config("API_LIST_ACTION") => "SemesterAntaraList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("semesterantaraview", $parm);
        } else {
            $url = $this->keyUrl("semesterantaraview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "semesterantaraadd?" . $parm;
        } else {
            $url = "semesterantaraadd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("semesterantaraedit", $parm);
        } else {
            $url = $this->keyUrl("semesterantaraedit", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("semesterantaralist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("semesterantaraadd", $parm);
        } else {
            $url = $this->keyUrl("semesterantaraadd", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("semesterantaralist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl(string $parm = ""): string
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("semesterantaradelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"id_smtr\":" . VarToJson($this->id_smtr->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->id_smtr->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->id_smtr->CurrentValue);
        } else {
    ...
    public function getRecordKeys(): array
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("id_smtr") ?? Route("id_smtr")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys(bool $setCurrent = true): string
    {
        ...
        foreach ($arKeys as $key) {
            if ($setCurrent) {
                $this->id_smtr->CurrentValue = $key;
            } else {
                $this->id_smtr->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->id_smtr);
                    $doc->exportCaption($this->Semester);
                    $doc->exportCaption($this->Jadwal);
        ...
        while (($row = $result->fetchAssociative()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->id_smtr);
                        $doc->exportField($this->Semester);
                        $doc->exportField($this->Jadwal);
