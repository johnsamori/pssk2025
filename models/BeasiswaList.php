 ...
 * Page class
 */
class BeasiswaList extends Beasiswa
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "BeasiswaList";
    ...
    public string $FormName = "fbeasiswalist";

    // CSS class/style
    public string $CurrentPageName = "beasiswalist";
    ...
    {
        $this->id_beasiswa->setVisibility();
        $this->Nama_Beasiswa->setVisibility();
    }
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Table object (beasiswa)
        if (!isset($GLOBALS["beasiswa"]) || $GLOBALS["beasiswa"]::class == PROJECT_NAMESPACE . "beasiswa") {
            $GLOBALS["beasiswa"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "beasiswaadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "beasiswadelete";
        $this->MultiUpdateUrl = "beasiswaupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'beasiswa');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "beasiswaview"); // If View page, no primary button
                } else { // List page
    ...
    protected function hideFieldsForAddEdit(): void
    {
        if ($this->isAdd() || $this->isCopy() || $this->isGridAdd()) {
            $this->id_beasiswa->Visible = false;
        }
    }
...
// Is modal
		...
		if (ReadCookie('beasiswa_searchpanel') == 'notactive' || ReadCookie('beasiswa_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('beasiswa_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fbeasiswasrch");
        }
        $filterList = Concat($filterList, $this->id_beasiswa->AdvancedSearch->toJson(), ","); // Field id_beasiswa
        $filterList = Concat($filterList, $this->Nama_Beasiswa->AdvancedSearch->toJson(), ","); // Field Nama_Beasiswa
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fbeasiswasrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field id_beasiswa
        $this->id_beasiswa->AdvancedSearch->SearchValue = $filter["x_id_beasiswa"] ?? "";
        $this->id_beasiswa->AdvancedSearch->SearchOperator = $filter["z_id_beasiswa"] ?? "";
        $this->id_beasiswa->AdvancedSearch->SearchCondition = $filter["v_id_beasiswa"] ?? "";
        $this->id_beasiswa->AdvancedSearch->SearchValue2 = $filter["y_id_beasiswa"] ?? "";
        $this->id_beasiswa->AdvancedSearch->SearchOperator2 = $filter["w_id_beasiswa"] ?? "";
        $this->id_beasiswa->AdvancedSearch->save();

        // Field Nama_Beasiswa
        $this->Nama_Beasiswa->AdvancedSearch->SearchValue = $filter["x_Nama_Beasiswa"] ?? "";
        $this->Nama_Beasiswa->AdvancedSearch->SearchOperator = $filter["z_Nama_Beasiswa"] ?? "";
        $this->Nama_Beasiswa->AdvancedSearch->SearchCondition = $filter["v_Nama_Beasiswa"] ?? "";
        $this->Nama_Beasiswa->AdvancedSearch->SearchValue2 = $filter["y_Nama_Beasiswa"] ?? "";
        $this->Nama_Beasiswa->AdvancedSearch->SearchOperator2 = $filter["w_Nama_Beasiswa"] ?? "";
        $this->Nama_Beasiswa->AdvancedSearch->save();
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_beasiswa); // id_beasiswa
            $this->updateSort($this->Nama_Beasiswa); // Nama_Beasiswa
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id_beasiswa->setSort("");
                $this->Nama_Beasiswa->setSort("");
            }
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"beasiswa\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"beasiswa\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"beasiswa\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fbeasiswalist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fbeasiswalist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_beasiswa->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"beasiswa\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "id_beasiswa");
            $this->createColumnOption($option, "Nama_Beasiswa");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fbeasiswasrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fbeasiswasrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fbeasiswalist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_beasiswa", "data-rowtype" => RowType::ADD]);
...
// Load basic search values
    ...
    public function loadRowValues(array|bool|null $row = null): void
    {
        ...
        $this->id_beasiswa->setDbValue($row['id_beasiswa']);
        $this->Nama_Beasiswa->setDbValue($row['Nama_Beasiswa']);
    }
    ...
    {
        $row = [];
        $row['id_beasiswa'] = $this->id_beasiswa->DefaultValue;
        $row['Nama_Beasiswa'] = $this->Nama_Beasiswa->DefaultValue;
        return $row;
    }
    ...
    public function renderRow(): void
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            // id_beasiswa
            $this->id_beasiswa->ViewValue = $this->id_beasiswa->CurrentValue;

            // Nama_Beasiswa
            $this->Nama_Beasiswa->ViewValue = $this->Nama_Beasiswa->CurrentValue;

            // id_beasiswa
            $this->id_beasiswa->HrefValue = "";
            $this->id_beasiswa->TooltipValue = "";

            // Nama_Beasiswa
            $this->Nama_Beasiswa->HrefValue = "";
            $this->Nama_Beasiswa->TooltipValue = "";
        }
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('beasiswa_searchpanel') == 'notactive' || ReadCookie('beasiswa_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fbeasiswasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('beasiswa_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fbeasiswasrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fbeasiswasrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
