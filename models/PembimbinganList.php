 ...
 * Page class
 */
class PembimbinganList extends Pembimbingan
{
    use MessagesTrait;
    use FormTrait;
    ...
    // Page object name
    public string $PageObjName = "PembimbinganList";
    ...
    public string $FormName = "fpembimbinganlist";

    // CSS class/style
    public string $CurrentPageName = "pembimbinganlist";
    ...
    public function __construct(Language $language, AdvancedSecurity $security)
    {
        ...
        // Initialize URLs
        $this->AddUrl = "pembimbinganadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "pembimbingandelete";
        $this->MultiUpdateUrl = "pembimbinganupdate";
...
// Is modal
        ...
        if ($this->IsModal) {
            $this->FormName = "fpembimbingangrid";
        }
		...
		if (ReadCookie('pembimbingan_searchpanel') == 'notactive' || ReadCookie('pembimbingan_searchpanel') == "") {
			RemoveClass($this->SearchPanelClass, "show");
			AppendClass($this->SearchPanelClass, "collapse");
		} elseif (ReadCookie('pembimbingan_searchpanel') == 'active') {
    ...
    public function getFilterList(): string
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fpembimbingansrch");
        }
        $filterList = Concat($filterList, $this->id_pem->AdvancedSearch->toJson(), ","); // Field id_pem
        $filterList = Concat($filterList, $this->NIP_Dosen_Pembimbing->AdvancedSearch->toJson(), ","); // Field NIP_Dosen_Pembimbing
        $filterList = Concat($filterList, $this->NIM->AdvancedSearch->toJson(), ","); // Field NIM
        $filterList = Concat($filterList, $this->Catatan_Mahasiswa->AdvancedSearch->toJson(), ","); // Field Catatan_Mahasiswa
        $filterList = Concat($filterList, $this->Catatan_Dosen_Wali->AdvancedSearch->toJson(), ","); // Field Catatan_Dosen_Wali
        $filterList = Concat($filterList, $this->Rekomendasi_Unit_BK->AdvancedSearch->toJson(), ","); // Field Rekomendasi_Unit_BK
        $filterList = Concat($filterList, $this->Nilai_IP_Semester->AdvancedSearch->toJson(), ","); // Field Nilai_IP_Semester
        $filterList = Concat($filterList, $this->Nilai_IPK->AdvancedSearch->toJson(), ","); // Field Nilai_IPK
        $filterList = Concat($filterList, $this->Surat_Peringatan->AdvancedSearch->toJson(), ","); // Field Surat_Peringatan
        $filterList = Concat($filterList, $this->Surat_Pemberitahuan->AdvancedSearch->toJson(), ","); // Field Surat_Pemberitahuan
        $filterList = Concat($filterList, $this->Rekomendasi_Akhir->AdvancedSearch->toJson(), ","); // Field Rekomendasi_Akhir
    ...
    protected function processFilterList(): bool
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fpembimbingansrch", $filters);
    ...
    protected function restoreFilterList(): void
    {
        ...
        // Field id_pem
        $this->id_pem->AdvancedSearch->SearchValue = $filter["x_id_pem"] ?? "";
        $this->id_pem->AdvancedSearch->SearchOperator = $filter["z_id_pem"] ?? "";
        $this->id_pem->AdvancedSearch->SearchCondition = $filter["v_id_pem"] ?? "";
        $this->id_pem->AdvancedSearch->SearchValue2 = $filter["y_id_pem"] ?? "";
        $this->id_pem->AdvancedSearch->SearchOperator2 = $filter["w_id_pem"] ?? "";
        $this->id_pem->AdvancedSearch->save();

        // Field NIP_Dosen_Pembimbing
        $this->NIP_Dosen_Pembimbing->AdvancedSearch->SearchValue = $filter["x_NIP_Dosen_Pembimbing"] ?? "";
        $this->NIP_Dosen_Pembimbing->AdvancedSearch->SearchOperator = $filter["z_NIP_Dosen_Pembimbing"] ?? "";
        $this->NIP_Dosen_Pembimbing->AdvancedSearch->SearchCondition = $filter["v_NIP_Dosen_Pembimbing"] ?? "";
        $this->NIP_Dosen_Pembimbing->AdvancedSearch->SearchValue2 = $filter["y_NIP_Dosen_Pembimbing"] ?? "";
        $this->NIP_Dosen_Pembimbing->AdvancedSearch->SearchOperator2 = $filter["w_NIP_Dosen_Pembimbing"] ?? "";
        $this->NIP_Dosen_Pembimbing->AdvancedSearch->save();
        ...
        // Field Catatan_Mahasiswa
        $this->Catatan_Mahasiswa->AdvancedSearch->SearchValue = $filter["x_Catatan_Mahasiswa"] ?? "";
        $this->Catatan_Mahasiswa->AdvancedSearch->SearchOperator = $filter["z_Catatan_Mahasiswa"] ?? "";
        $this->Catatan_Mahasiswa->AdvancedSearch->SearchCondition = $filter["v_Catatan_Mahasiswa"] ?? "";
        $this->Catatan_Mahasiswa->AdvancedSearch->SearchValue2 = $filter["y_Catatan_Mahasiswa"] ?? "";
        $this->Catatan_Mahasiswa->AdvancedSearch->SearchOperator2 = $filter["w_Catatan_Mahasiswa"] ?? "";
        $this->Catatan_Mahasiswa->AdvancedSearch->save();

        // Field Catatan_Dosen_Wali
        $this->Catatan_Dosen_Wali->AdvancedSearch->SearchValue = $filter["x_Catatan_Dosen_Wali"] ?? "";
        $this->Catatan_Dosen_Wali->AdvancedSearch->SearchOperator = $filter["z_Catatan_Dosen_Wali"] ?? "";
        $this->Catatan_Dosen_Wali->AdvancedSearch->SearchCondition = $filter["v_Catatan_Dosen_Wali"] ?? "";
        $this->Catatan_Dosen_Wali->AdvancedSearch->SearchValue2 = $filter["y_Catatan_Dosen_Wali"] ?? "";
        $this->Catatan_Dosen_Wali->AdvancedSearch->SearchOperator2 = $filter["w_Catatan_Dosen_Wali"] ?? "";
        $this->Catatan_Dosen_Wali->AdvancedSearch->save();

        // Field Rekomendasi_Unit_BK
        $this->Rekomendasi_Unit_BK->AdvancedSearch->SearchValue = $filter["x_Rekomendasi_Unit_BK"] ?? "";
        $this->Rekomendasi_Unit_BK->AdvancedSearch->SearchOperator = $filter["z_Rekomendasi_Unit_BK"] ?? "";
        $this->Rekomendasi_Unit_BK->AdvancedSearch->SearchCondition = $filter["v_Rekomendasi_Unit_BK"] ?? "";
        $this->Rekomendasi_Unit_BK->AdvancedSearch->SearchValue2 = $filter["y_Rekomendasi_Unit_BK"] ?? "";
        $this->Rekomendasi_Unit_BK->AdvancedSearch->SearchOperator2 = $filter["w_Rekomendasi_Unit_BK"] ?? "";
        $this->Rekomendasi_Unit_BK->AdvancedSearch->save();

        // Field Nilai_IP_Semester
        $this->Nilai_IP_Semester->AdvancedSearch->SearchValue = $filter["x_Nilai_IP_Semester"] ?? "";
        $this->Nilai_IP_Semester->AdvancedSearch->SearchOperator = $filter["z_Nilai_IP_Semester"] ?? "";
        $this->Nilai_IP_Semester->AdvancedSearch->SearchCondition = $filter["v_Nilai_IP_Semester"] ?? "";
        $this->Nilai_IP_Semester->AdvancedSearch->SearchValue2 = $filter["y_Nilai_IP_Semester"] ?? "";
        $this->Nilai_IP_Semester->AdvancedSearch->SearchOperator2 = $filter["w_Nilai_IP_Semester"] ?? "";
        $this->Nilai_IP_Semester->AdvancedSearch->save();

        // Field Nilai_IPK
        $this->Nilai_IPK->AdvancedSearch->SearchValue = $filter["x_Nilai_IPK"] ?? "";
        $this->Nilai_IPK->AdvancedSearch->SearchOperator = $filter["z_Nilai_IPK"] ?? "";
        $this->Nilai_IPK->AdvancedSearch->SearchCondition = $filter["v_Nilai_IPK"] ?? "";
        $this->Nilai_IPK->AdvancedSearch->SearchValue2 = $filter["y_Nilai_IPK"] ?? "";
        $this->Nilai_IPK->AdvancedSearch->SearchOperator2 = $filter["w_Nilai_IPK"] ?? "";
        $this->Nilai_IPK->AdvancedSearch->save();

        // Field Surat_Peringatan
        $this->Surat_Peringatan->AdvancedSearch->SearchValue = $filter["x_Surat_Peringatan"] ?? "";
        $this->Surat_Peringatan->AdvancedSearch->SearchOperator = $filter["z_Surat_Peringatan"] ?? "";
        $this->Surat_Peringatan->AdvancedSearch->SearchCondition = $filter["v_Surat_Peringatan"] ?? "";
        $this->Surat_Peringatan->AdvancedSearch->SearchValue2 = $filter["y_Surat_Peringatan"] ?? "";
        $this->Surat_Peringatan->AdvancedSearch->SearchOperator2 = $filter["w_Surat_Peringatan"] ?? "";
        $this->Surat_Peringatan->AdvancedSearch->save();

        // Field Surat_Pemberitahuan
        $this->Surat_Pemberitahuan->AdvancedSearch->SearchValue = $filter["x_Surat_Pemberitahuan"] ?? "";
        $this->Surat_Pemberitahuan->AdvancedSearch->SearchOperator = $filter["z_Surat_Pemberitahuan"] ?? "";
        $this->Surat_Pemberitahuan->AdvancedSearch->SearchCondition = $filter["v_Surat_Pemberitahuan"] ?? "";
        $this->Surat_Pemberitahuan->AdvancedSearch->SearchValue2 = $filter["y_Surat_Pemberitahuan"] ?? "";
        $this->Surat_Pemberitahuan->AdvancedSearch->SearchOperator2 = $filter["w_Surat_Pemberitahuan"] ?? "";
        $this->Surat_Pemberitahuan->AdvancedSearch->save();

        // Field Rekomendasi_Akhir
        $this->Rekomendasi_Akhir->AdvancedSearch->SearchValue = $filter["x_Rekomendasi_Akhir"] ?? "";
        $this->Rekomendasi_Akhir->AdvancedSearch->SearchOperator = $filter["z_Rekomendasi_Akhir"] ?? "";
        $this->Rekomendasi_Akhir->AdvancedSearch->SearchCondition = $filter["v_Rekomendasi_Akhir"] ?? "";
        $this->Rekomendasi_Akhir->AdvancedSearch->SearchValue2 = $filter["y_Rekomendasi_Akhir"] ?? "";
        $this->Rekomendasi_Akhir->AdvancedSearch->SearchOperator2 = $filter["w_Rekomendasi_Akhir"] ?? "";
        $this->Rekomendasi_Akhir->AdvancedSearch->save();
    ...
    public function basicSearchWhere(bool $default = false): string
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->NIP_Dosen_Pembimbing;
        $searchFlds[] = &$this->NIM;
        $searchFlds[] = &$this->Catatan_Mahasiswa;
        $searchFlds[] = &$this->Catatan_Dosen_Wali;
        $searchFlds[] = &$this->Rekomendasi_Unit_BK;
        $searchFlds[] = &$this->Nilai_IP_Semester;
        $searchFlds[] = &$this->Nilai_IPK;
        $searchFlds[] = &$this->Surat_Peringatan;
        $searchFlds[] = &$this->Surat_Pemberitahuan;
        $searchFlds[] = &$this->Rekomendasi_Akhir;
        $searchFlds[] = &$this->Ip;
    ...
    protected function setupSortOrder(): void
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->id_pem); // id_pem
            $this->updateSort($this->NIP_Dosen_Pembimbing); // NIP_Dosen_Pembimbing
            $this->updateSort($this->NIM); // NIM
            $this->updateSort($this->Catatan_Mahasiswa); // Catatan_Mahasiswa
            $this->updateSort($this->Catatan_Dosen_Wali); // Catatan_Dosen_Wali
            $this->updateSort($this->Rekomendasi_Unit_BK); // Rekomendasi_Unit_BK
            $this->updateSort($this->Nilai_IP_Semester); // Nilai_IP_Semester
            $this->updateSort($this->Nilai_IPK); // Nilai_IPK
            $this->updateSort($this->Surat_Peringatan); // Surat_Peringatan
            $this->updateSort($this->Surat_Pemberitahuan); // Surat_Pemberitahuan
            $this->updateSort($this->Rekomendasi_Akhir); // Rekomendasi_Akhir
    ...
    protected function resetCmd(): void
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id_pem->setSort("");
                $this->NIP_Dosen_Pembimbing->setSort("");
                $this->NIM->setSort("");
                $this->Catatan_Mahasiswa->setSort("");
                $this->Catatan_Dosen_Wali->setSort("");
                $this->Rekomendasi_Unit_BK->setSort("");
                $this->Nilai_IP_Semester->setSort("");
                $this->Nilai_IPK->setSort("");
                $this->Surat_Peringatan->setSort("");
                $this->Surat_Pemberitahuan->setSort("");
                $this->Rekomendasi_Akhir->setSort("");
    ...
    public function renderListOptions(): void
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($this->security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"pembimbingan\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $this->language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($this->security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"pembimbingan\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-ask=\"1\" data-btn=\"SaveBtn\">" . $this->language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($this->security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
					$opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"pembimbingan\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("CopyLink") . "</a>";
                } else {
            ...
            foreach ($this->ListActions as $listAction) { // ActionType::SINGLE
                ...
                if ($listAction->Select == ActionType::SINGLE && $listAction->getVisible()) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                        "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpembimbinganlist\" data-key=\"" . $this->keyToJson(true) .
                    ...
                    if ($body == "") { // Setup first button
                        $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action" . ($listAction->getEnabled() ? "" : " disabled") .
                            "\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fpembimbinganlist\" data-key=\"" . $this->keyToJson(true) .
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->id_pem->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions(): void
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
			$item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"pembimbingan\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-ask=\"1\" data-btn=\"AddBtn\">" . $this->language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "id_pem");
            $this->createColumnOption($option, "NIP_Dosen_Pembimbing");
            $this->createColumnOption($option, "NIM");
            $this->createColumnOption($option, "Catatan_Mahasiswa");
            $this->createColumnOption($option, "Catatan_Dosen_Wali");
            $this->createColumnOption($option, "Rekomendasi_Unit_BK");
            $this->createColumnOption($option, "Nilai_IP_Semester");
            $this->createColumnOption($option, "Nilai_IPK");
            $this->createColumnOption($option, "Surat_Peringatan");
            $this->createColumnOption($option, "Surat_Pemberitahuan");
            $this->createColumnOption($option, "Rekomendasi_Akhir");
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fpembimbingansrch\" data-ew-action=\"none\">" . $this->language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fpembimbingansrch\" data-ew-action=\"none\">" . $this->language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions(): void
    {
        ...
        foreach ($this->ListActions as $listAction) { // ActionType::MULTIPLE
            if ($listAction->Select == ActionType::MULTIPLE && $listAction->getVisible()) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fpembimbinganlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
                $item->Visible = true;
            }
        }
    ...
    public function setupRow(): void
    {
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_pembimbingan", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_pembimbingan",
...
// Load basic search values
	...
	// Begin of add Search Panel Status by Masino Sinaga, October 13, 2024
		...
		if (ReadCookie('pembimbingan_searchpanel') == 'notactive' || ReadCookie('pembimbingan_searchpanel') == "") {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpembimbingansrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		} elseif (ReadCookie('pembimbingan_searchpanel') == 'active') {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle active\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpembimbingansrch\" aria-pressed=\"true\">" . $this->language->phrase("SearchLink") . "</a>";
		} else {
			$item->Body = "<a class=\"btn btn-default ew-search-toggle\" role=\"button\" title=\"" . $this->language->phrase("SearchPanel") . "\" data-caption=\"" . $this->language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fpembimbingansrch\" aria-pressed=\"false\">" . $this->language->phrase("SearchLink") . "</a>";
		}
        $item->Visible = true;
